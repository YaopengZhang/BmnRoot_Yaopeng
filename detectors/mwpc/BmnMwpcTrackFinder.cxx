...
//        if (fDebug)cout<<" dx_loc "<<dx_loc<<" dy_loc "<<dy_loc<<endl;
        ...
        if (Chi2_m < min_Chi2m && Nvariat < Nvariations && fabs(dx_loc) < 1.5 && fabs(dy_loc) < 2.) {//09.2019
...
InitStatus BmnMwpcTrackFinder::Init() {
  ...
  for (int i=0; i < kNumPairs; ++i) {
    if (fDebug){
      ...
      TH2D *h1;
      h1 = new TH2D(Form("YvsX_pair%d",i), Form("YvsX_pair%d; X[cm]; Y[cm]",i),  200, -10.,10., 200, -10.,10.);fList.Add(h1);hYvsX_pair.push_back(h1);
      h1 = new TH2D(Form("Y_X_in_target_pair%d",i), Form("posY vs posX pair%d in target; X[cm]; Y[cm]",i), 200, -10.,10., 200, -10.,10.);fList.Add(h1);hY_X_in_target_pair.push_back(h1);
  ...
  for(Int_t i = 0; i < kNumPairs; i++) {
    ...
    */
    ...
    if (fRunPeriod == 8){
      ...
      //pair0
      shift_pair[0][0] =  -0.0023;//Ax
      shift_pair[0][1] =  -.41 -0.3;//x
      shift_pair[0][2] =  -0.0048;//Ay
      shift_pair[0][3] =   1.518;//y
      //pair1
      shift_pair[1][0] =  0.;//Ax
      shift_pair[1][1] =  0.;//x
      shift_pair[1][2] =  0.;//Ay
