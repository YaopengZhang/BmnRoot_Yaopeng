...
void BmnFieldMap::Init() {
    ...
    if (!fFileName.Contains("Extrap")) {
        LOG(debug) << "NEW FIELD MAP IS USED";
        ReadRootFileNewFormat(fFileName);
    } else {
        LOG(debug) << "OLD FIELD MAP IS USED";
...
void BmnFieldMap::ReadRootFileNewFormat(const char* fileName) {
    TString* type = 0;
    coordinate_info_t X;
    coordinate_info_t Y;
    coordinate_info_t Z;
    ...
    // Open root file
    LOG(info) << "BmnFieldMap: Reading field map from ROOT file " << fileName;
