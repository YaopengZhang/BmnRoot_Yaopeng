...
// found on file: srcsim_alexei.root
//////////////////////////////////////////////////////////

#ifndef Read_srcsim_alexei_h
#define Read_srcsim_alexei_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   Read_srcsim_alexei(TTree *tree=0);
   virtual ~Read_srcsim_alexei();
...
#ifdef Read_srcsim_alexei_cxx
Read_srcsim_alexei::Read_srcsim_alexei(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("srcsim_alexei.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("srcsim_alexei.root");
      }
      // f->GetObject("bmndata",tree);
      // TFile *f2 = (TFile*)gROOT->GetListOfFiles()->FindObject("srcsim_alexei_1.root");
      // if (!f2 || !f2->IsOpen()) {
      //    f2 = new TFile("srcsim_alexei_1.root");
      // }

      // TFile *f3 = (TFile*)gROOT->GetListOfFiles()->FindObject("srcsim_alexei_2.root");
      // if (!f3 || !f3->IsOpen()) {
      //    f3 = new TFile("srcsim_alexei_2.root");
      // }

      // TFile *f4 = (TFile*)gROOT->GetListOfFiles()->FindObject("srcsim_alexei_3.root");
      // if (!f4 || !f4->IsOpen()) {
      //    f4 = new TFile("srcsim_alexei_3.root");
      // }
      ...
      chain->Add("srcsim_alexei.root");
      // chain->Add("srcsim_alexei_1.root");
      // chain->Add("srcsim_alexei_2.root");
      // chain->Add("srcsim_alexei_3.root");
...
Read_srcsim_alexei::~Read_srcsim_alexei()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t Read_srcsim_alexei::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t Read_srcsim_alexei::LoadTree(Long64_t entry)
{
...
void Read_srcsim_alexei::Init(TTree *tree)
{
...
Bool_t Read_srcsim_alexei::Notify()
{
...
void Read_srcsim_alexei::Show(Long64_t entry)
{
...
Int_t Read_srcsim_alexei::Cut(Long64_t entry)
{
...
#endif // #ifdef Read_srcsim_alexei_cxx
