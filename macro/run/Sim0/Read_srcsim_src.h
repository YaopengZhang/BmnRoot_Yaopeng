...
// found on file: srcsim_src.root
//////////////////////////////////////////////////////////

#ifndef Read_srcsim_src_h
#define Read_srcsim_src_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   Read_srcsim_src(TTree *tree=0);
   virtual ~Read_srcsim_src();
...
#ifdef Read_srcsim_src_cxx
Read_srcsim_src::Read_srcsim_src(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("srcsim_src.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("srcsim_src.root");
      }
      // f->GetObject("bmndata",tree);
      TFile *f2 = (TFile*)gROOT->GetListOfFiles()->FindObject("srcsim_src_1.root");
      if (!f2 || !f2->IsOpen()) {
         f2 = new TFile("srcsim_src_1.root");
      }

      TFile *f3 = (TFile*)gROOT->GetListOfFiles()->FindObject("srcsim_src_2.root");
      if (!f3 || !f3->IsOpen()) {
         f3 = new TFile("srcsim_src_2.root");
      }

      TFile *f4 = (TFile*)gROOT->GetListOfFiles()->FindObject("srcsim_src_3.root");
      if (!f4 || !f4->IsOpen()) {
         f4 = new TFile("srcsim_src_3.root");
      }
      ...
      chain->Add("srcsim_src.root");
      chain->Add("srcsim_src_1.root");
      chain->Add("srcsim_src_2.root");
      chain->Add("srcsim_src_3.root");
...
Read_srcsim_src::~Read_srcsim_src()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t Read_srcsim_src::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t Read_srcsim_src::LoadTree(Long64_t entry)
{
...
void Read_srcsim_src::Init(TTree *tree)
{
...
void Read_srcsim_src::Show(Long64_t entry)
{
...
Int_t Read_srcsim_src::Cut(Long64_t entry)
{
...
#endif // #ifdef Read_srcsim_src_cxx
