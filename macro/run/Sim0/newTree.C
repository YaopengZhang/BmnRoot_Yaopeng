...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   fChain->SetBranchStatus("pRec",1);
   fChain->SetBranchStatus("rec_type",1);
   ...
   Int_t Multiplicity = 3, PDG_code = 2212; // 2212 proton; 2112 neutron
   Double_t vertex_x, vertex_y, vertex_z; // cm
   Double_t Weight;

   TFile *f = new TFile("./vertex_SRC.root","RECREATE"); // "UPDATE"
   ...
   ofstream OUT("Generator_input_SRC.txt"); // Generate the input file for FairAsciiGenerator
   ...
   // for (Long64_t jentry=0; jentry<10;jentry++) {
      ...
      OUT<<rec_type<<"\t"<<pRec[0]<<"\t"<<pRec[1]<<"\t"<<pRec[2]<<"\n";
      //-------------------------------Generator_input.txt-----------------------------------
