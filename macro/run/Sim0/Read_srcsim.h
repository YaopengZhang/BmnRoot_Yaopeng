...
// found on file: srcsim.root
//////////////////////////////////////////////////////////

#ifndef Read_srcsim_h
#define Read_srcsim_h
...
#ifdef Read_srcsim_cxx
Read_srcsim::Read_srcsim(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("srcsim.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("srcsim.root");
      }
      // f->GetObject("bmndata",tree);
      TFile *f2 = (TFile*)gROOT->GetListOfFiles()->FindObject("srcsim_1.root");
      if (!f2 || !f2->IsOpen()) {
         f2 = new TFile("srcsim_1.root");
      }
      ...
      chain->Add("srcsim.root");
      chain->Add("srcsim_1.root");
...
Read_srcsim::~Read_srcsim()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t Read_srcsim::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t Read_srcsim::LoadTree(Long64_t entry)
{
...
void Read_srcsim::Init(TTree *tree)
{
...
void Read_srcsim::Show(Long64_t entry)
{
...
Int_t Read_srcsim::Cut(Long64_t entry)
{
