...
// found on file: srcsim_delta_test.root
//////////////////////////////////////////////////////////

#ifndef Read_srcsim_delta_test_h
#define Read_srcsim_delta_test_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   Read_srcsim_delta_test(TTree *tree=0);
   virtual ~Read_srcsim_delta_test();
...
#ifdef Read_srcsim_delta_test_cxx
Read_srcsim_delta_test::Read_srcsim_delta_test(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("srcsim_delta_test.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("srcsim_delta_test.root");
      }
      // f->GetObject("bmndata",tree);
      // TFile *f2 = (TFile*)gROOT->GetListOfFiles()->FindObject("srcsim_delta_test_1.root");
      // if (!f2 || !f2->IsOpen()) {
      //    f2 = new TFile("srcsim_delta_test_1.root");
      // }
      ...
      chain->Add("srcsim_delta_test.root");
      // chain->Add("srcsim_delta_test_1.root");
...
Read_srcsim_delta_test::~Read_srcsim_delta_test()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t Read_srcsim_delta_test::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t Read_srcsim_delta_test::LoadTree(Long64_t entry)
{
...
void Read_srcsim_delta_test::Init(TTree *tree)
{
...
Bool_t Read_srcsim_delta_test::Notify()
{
...
void Read_srcsim_delta_test::Show(Long64_t entry)
{
...
Int_t Read_srcsim_delta_test::Cut(Long64_t entry)
{
...
#endif // #ifdef Read_srcsim_delta_test_cxx
