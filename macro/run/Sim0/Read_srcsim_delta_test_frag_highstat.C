#define Read_srcsim_delta_test_frag_highstat_cxx
#include "Read_srcsim_delta_test_frag_highstat.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
...
int If_delta(int EventNo, TVector3 p1, TVector3 p2, TString FileName){
   int delta_flag = 0;
   int No_exist_flag = 0;
   ...
   while (std::getline(file, line)) {
      ...
      int NN = int(numbers[0]);

      if(NN<EventNo) continue;

      if(NN==EventNo){
         No_exist_flag = 1;
         TVector3 vec(numbers[1], numbers[2], numbers[3]);
         // cout<<vec.X()<<"\t"<<vec.Y()<<"\t"<<vec.Z()<<"\t"<<endl;
   
         double dotProduct_p1 = vec.Dot(p1);
         double dotProduct_p2 = vec.Dot(p2);

         double magnitude = vec.Mag();
         double magnitude1 = p1.Mag();
         double magnitude2 = p2.Mag();

         double cosTheta_p1 = dotProduct_p1 / (magnitude * magnitude1);
         double cosTheta_p2 = dotProduct_p2 / (magnitude * magnitude2);

         double angleRad_p1 = acos(cosTheta_p1);
         double angleRad_p2 = acos(cosTheta_p2);

         double angleDeg_p1 = angleRad_p1 * 180.0 / TMath::Pi();
         double angleDeg_p2 = angleRad_p2 * 180.0 / TMath::Pi();

         double amp_diff_p1 = fabs(magnitude1-magnitude)/magnitude1;
         double amp_diff_p2 = fabs(magnitude2-magnitude)/magnitude2;

         // std::cout << "Amp difference: " << amp_diff_p1 <<"\t"<< amp_diff_p2 << std::endl;
         // std::cout << "Angle (degrees): " << angleDeg_p1 <<"\t"<< angleDeg_p2 << std::endl;

         if( (angleDeg_p1<20 && amp_diff_p1<0.1) || (angleDeg_p2<20 && amp_diff_p2<0.1) ) delta_flag = 1;
      }

      if(NN>EventNo && No_exist_flag==0){
         delta_flag = 0;
         break;
      }
      if(NN>EventNo) break;

      // std::cout << "Line contains " << numbers.size() << " values: ";
...
// void test(){
//    TVector3 vec1(-1.1469,0.076894,2.7941);
//    TVector3 vec2(0, 1, 0);
//    cout<<If_delta(4, vec1, vec2, "Delta_events_delta_test_frag_highstat.txt")<<endl;
// }

Double_t Read_srcsim_delta_test_frag_highstat::Loop(Double_t Boost_resolution, Double_t TOF_resolution)
{
//   In a ROOT session, you can do:
//      root> .L Read_srcsim_delta_test_frag_highstat.C
//      root> Read_srcsim_delta_test_frag_highstat t
...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   TChain ch("T");
   ch.Add("gen_delta_test_frag_highstat.root");
   ...
   ch_vertex.Add("vertex_delta_test_frag_highstat.root");
   ...
   double P_miss_x, P_miss_y, P_miss_z, P_miss_E, P_miss, E_miss, M_miss, theta1, theta2, phi1, phi2;
   ...
   std::vector<int> EvtNo_11B;

   std::ifstream file("Delta_events_delta_test_frag_highstat_11B.txt");
   if (!file.is_open()) {
      std::cerr << "Can't open the input file!" << std::endl;
      return -1;
   }
   ...
   while (std::getline(file, line)) {
      ...
      int value;
      if(ss >> value) {
         EvtNo_11B.push_back(value);
      }
   }

   Int_t Numof11B = EvtNo_11B.size();
   Int_t Current11BNo = 0;



   std::vector<int> EvtNo_delta;
   std::vector<double> delta_x, delta_y, delta_z;
   std::ifstream file_delta("Delta_events_delta_test_frag_highstat.txt");
   if (!file_delta.is_open()) {
      std::cerr << "Can't open the input file!" << std::endl;
      return 1;
   }

   std::string line_delta;
   while (std::getline(file_delta, line_delta)) {
      std::stringstream ss(line_delta);

      int runNo;
      double value[3];
      while (ss >> runNo >> value[0] >> value[1] >> value[2]) {
         EvtNo_delta.push_back(runNo);
         delta_x.push_back(value[0]);
         delta_y.push_back(value[1]);
         delta_z.push_back(value[2]);
      }
      // cout<<runNo<<"\t"<<value[0]<<"\t"<<value[1]<<"\t"<<value[2]<<endl;
      // break;

   }
   Int_t Numofdelta = EvtNo_delta.size();
   Int_t CurrentdeltaNo = 0;
   ...
   TH1F *h_Mandelstam_t = new TH1F("h_Mandelstam_t","Mandelstam variable t",50,0,7);
   TH1F *h_Mandelstam_t_1 = new TH1F("h_Mandelstam_t_1","Mandelstam variable t",50,0,7);
   ...
   TH1F *h_Mandelstam_u = new TH1F("h_Mandelstam_u","Mandelstam variable u",50,0,7);
   TH1F *h_Mandelstam_u_1 = new TH1F("h_Mandelstam_u_1","Mandelstam variable u",50,0,7);
   ...
   TH1F* h_Mandelstam_u_t_1 = new TH1F("h_Mandelstam_u_t_1","Mandelstam variable Max{|u|, |t|}",50,0,7);
   TH1F* h_Mandelstam_u_t = new TH1F("h_Mandelstam_u_t","Mandelstam variable Max{|u|, |t|}",50,0,7);
   ...
   TH1F* h_Mandelstam_u_t_min_1 = new TH1F("h_Mandelstam_u_t_min_1","Mandelstam variable Min{|u|, |t|}",50,0,7);
   TH1F* h_Mandelstam_u_t_min = new TH1F("h_Mandelstam_u_t_min","Mandelstam variable Min{|u|, |t|}",50,0,7);
   ...
   // TH1F *h_p1x = new TH1F("p1x","P1_{ x} in lab  system",200,-1,3);
   // TH1F *h_p1x_1 = new TH1F("p1x_1","P1_{ x} in lab  system",200,-1,3);
   TH1F *h_p1x = new TH1F("p1x","P1_{ x} in lab  system",100,0.5,1.5);
   TH1F *h_p1x_1 = new TH1F("p1x_1","P1_{ x} in lab  system",100,0.5,1.5);
   ...
   TH1F *h_p1y = new TH1F("p1y","P1_{ y} in lab system",100,-1,1);
   TH1F *h_p1y_1 = new TH1F("p1y_1","P1_{ y} in lab system",100,-1,1);
   ...
   TH1F *h_p1z = new TH1F("p1z","P1_{ z} in lab reference system",100,0.5,4);
   TH1F *h_p1z_1 = new TH1F("p1z_1","P1_{ z} in lab reference system",100,0.5,4);
   ...
   TH1F *h_E1 = new TH1F("P1","P1 in lab system",100,1,4);
   TH1F *h_E1_1 = new TH1F("P1_1","P1 in lab system",100,1,4);
   ...
   TH1F *h_p2x = new TH1F("p2x","P2_{ x} in lab system",100,-1.5,-0.5);
   TH1F *h_p2x_1 = new TH1F("p2x_1","P2_{ x} in lab system",100,-1.5,-0.5);
   ...
   TH1F *h_p2y = new TH1F("p2y","P2_{ y} in lab system",100,-1,1);
   TH1F *h_p2y_1 = new TH1F("p2y_1","P2_{ y} in lab system",100,-1,1);
   ...
   TH1F *h_p2z = new TH1F("p2z","P2_{ z} in lab system",100,0.5,4);
   TH1F *h_p2z_1 = new TH1F("p2z_1","P2_{ z} in lab system",100,0.5,4);
   ...
   // TH1F *h_E2 = new TH1F("P2","P2 in lab system",100,0.5,4.5);
   // TH1F *h_E2_1 = new TH1F("P2_1","P2 in lab system",100,0.5,4.5);
   TH1F *h_E2 = new TH1F("P2","P2 in lab system",100,1,4);
   TH1F *h_E2_1 = new TH1F("P2_1","P2 in lab system",100,1,4);
   ...
   TH1F *h_p1_minus_p2 = new TH1F("p1_minus_p2","P_{1} - P_{2} in lab  system",150,-4,4);
   h_p1_minus_p2->SetXTitle("P_{1} - P_{2} / GeV");
	h_p1_minus_p2->GetXaxis()->CenterTitle(1);
	h_p1_minus_p2->GetXaxis()->SetTitleFont(32);
   h_p1_minus_p2->GetXaxis()->SetTitleSize(0.06);
	h_p1_minus_p2->GetXaxis()->SetTitleOffset(0.7);
   h_p1_minus_p2->SetYTitle("Counts");
	h_p1_minus_p2->GetYaxis()->CenterTitle(1);
	h_p1_minus_p2->GetYaxis()->SetTitleFont(32);
   h_p1_minus_p2->GetYaxis()->SetTitleSize(0.04);

   TH1F *h_p1x_minus_p2x = new TH1F("p1x_minus_p2x","P_{1, x} - P_{2, x} in lab  system",150,1,3);
   h_p1x_minus_p2x->SetXTitle("P_{1, x} - P_{2, x} / GeV");
	h_p1x_minus_p2x->GetXaxis()->CenterTitle(1);
	h_p1x_minus_p2x->GetXaxis()->SetTitleFont(32);
   h_p1x_minus_p2x->GetXaxis()->SetTitleSize(0.06);
	h_p1x_minus_p2x->GetXaxis()->SetTitleOffset(0.7);
   h_p1x_minus_p2x->SetYTitle("Counts");
	h_p1x_minus_p2x->GetYaxis()->CenterTitle(1);
	h_p1x_minus_p2x->GetYaxis()->SetTitleFont(32);
   h_p1x_minus_p2x->GetYaxis()->SetTitleSize(0.04);

   TH1F *h_p1y_minus_p2y = new TH1F("p1y_minus_p2y","P_{1, y} - P_{2, y} in lab  system",150,-2,2);
   h_p1y_minus_p2y->SetXTitle("P_{1, y} - P_{2, y} / GeV");
	h_p1y_minus_p2y->GetXaxis()->CenterTitle(1);
	h_p1y_minus_p2y->GetXaxis()->SetTitleFont(32);
   h_p1y_minus_p2y->GetXaxis()->SetTitleSize(0.06);
	h_p1y_minus_p2y->GetXaxis()->SetTitleOffset(0.7);
   h_p1y_minus_p2y->SetYTitle("Counts");
	h_p1y_minus_p2y->GetYaxis()->CenterTitle(1);
	h_p1y_minus_p2y->GetYaxis()->SetTitleFont(32);
   h_p1y_minus_p2y->GetYaxis()->SetTitleSize(0.04);

   TH1F *h_p1z_minus_p2z = new TH1F("p1z_minus_p2z","P_{1, z} - P_{2, z} in lab  system",150,-4,4);
   h_p1z_minus_p2z->SetXTitle("P_{1, z} - P_{2, z} / GeV");
	h_p1z_minus_p2z->GetXaxis()->CenterTitle(1);
	h_p1z_minus_p2z->GetXaxis()->SetTitleFont(32);
   h_p1z_minus_p2z->GetXaxis()->SetTitleSize(0.06);
	h_p1z_minus_p2z->GetXaxis()->SetTitleOffset(0.7);
   h_p1z_minus_p2z->SetYTitle("Counts");
	h_p1z_minus_p2z->GetYaxis()->CenterTitle(1);
	h_p1z_minus_p2z->GetYaxis()->SetTitleFont(32);
   h_p1z_minus_p2z->GetYaxis()->SetTitleSize(0.04);

   TH1F *h_p1_and_p2 = new TH1F("p1_and_p2","P1 + P2 in lab system",100,0.5,4.5);
   h_p1_and_p2->SetXTitle("P_{1} + P_{2} / GeV");
	h_p1_and_p2->GetXaxis()->CenterTitle(1);
	h_p1_and_p2->GetXaxis()->SetTitleFont(32);
   h_p1_and_p2->GetXaxis()->SetTitleSize(0.06);
	h_p1_and_p2->GetXaxis()->SetTitleOffset(0.7);
   h_p1_and_p2->SetYTitle("Counts");
	h_p1_and_p2->GetYaxis()->CenterTitle(1);
	h_p1_and_p2->GetYaxis()->SetTitleFont(32);
   h_p1_and_p2->GetYaxis()->SetTitleSize(0.04);

   TH1F *h_p1x_and_p2x = new TH1F("p1x_and_p2x","P_{1, x} + P_{2, x} in lab system",150,-1,1);
   h_p1x_and_p2x->SetXTitle("P_{1, x} + P_{2, x} / GeV");
	h_p1x_and_p2x->GetXaxis()->CenterTitle(1);
	h_p1x_and_p2x->GetXaxis()->SetTitleFont(32);
   h_p1x_and_p2x->GetXaxis()->SetTitleSize(0.06);
	h_p1x_and_p2x->GetXaxis()->SetTitleOffset(0.7);
   h_p1x_and_p2x->SetYTitle("Counts");
	h_p1x_and_p2x->GetYaxis()->CenterTitle(1);
	h_p1x_and_p2x->GetYaxis()->SetTitleFont(32);
   h_p1x_and_p2x->GetYaxis()->SetTitleSize(0.04);

   TH1F *h_p1y_and_p2y = new TH1F("p1y_and_p2y","P_{1, y} + P_{2, y} in lab system",150,-1,1);
   h_p1y_and_p2y->SetXTitle("P_{1, y} + P_{2, y} / GeV");
	h_p1y_and_p2y->GetXaxis()->CenterTitle(1);
	h_p1y_and_p2y->GetXaxis()->SetTitleFont(32);
   h_p1y_and_p2y->GetXaxis()->SetTitleSize(0.06);
	h_p1y_and_p2y->GetXaxis()->SetTitleOffset(0.7);
   h_p1y_and_p2y->SetYTitle("Counts");
	h_p1y_and_p2y->GetYaxis()->CenterTitle(1);
	h_p1y_and_p2y->GetYaxis()->SetTitleFont(32);
   h_p1y_and_p2y->GetYaxis()->SetTitleSize(0.04);

   TH1F *h_p1z_and_p2z = new TH1F("p1z_and_p2z","P_{1, z} + P_{2, z} in lab system",150,2,6);
   h_p1z_and_p2z->SetXTitle("P_{1, z} + P_{2, z} / GeV");
	h_p1z_and_p2z->GetXaxis()->CenterTitle(1);
	h_p1z_and_p2z->GetXaxis()->SetTitleFont(32);
   h_p1z_and_p2z->GetXaxis()->SetTitleSize(0.06);
	h_p1z_and_p2z->GetXaxis()->SetTitleOffset(0.7);
   h_p1z_and_p2z->SetYTitle("Counts");
	h_p1z_and_p2z->GetYaxis()->CenterTitle(1);
	h_p1z_and_p2z->GetYaxis()->SetTitleFont(32);
   h_p1z_and_p2z->GetYaxis()->SetTitleSize(0.04);

   TH1F *h_beta_left = new TH1F("beta_left","#beta for left particle",100,0.7,1.);
   ...
   TH1F *h_beta_right = new TH1F("beta_right","#beta for right particle",100,0.7,1.);
   ...
   TH1F *h_p1xp2x_1 = new TH1F("p_miss_x_1","Pmiss_{ x} from generator in ^{12}C reference system",100,-1,1);
   TH1F *h_p1xp2x_2 = new TH1F("p_miss_x_2","Pmiss_{ x} in ^{12}C reference system",100,-1,1);
   ...
   TH1F *h_p1yp2y_1 = new TH1F("p_miss_y_1","Pmiss_{ y} from generator in ^{12}C reference system",100,-1,1);
   TH1F *h_p1yp2y_2 = new TH1F("p_miss_y_2","Pmiss_{ y}  in ^{12}C reference system",100,-1,1);
   ...
   TH1F *h_p1zp2z_1 = new TH1F("p_miss_z_1","Pmiss_{ z} in ^{12}C reference system",100,-1,1);
   TH1F *h_p1zp2z_2 = new TH1F("p_miss_z_2","Pmiss_{ z} in ^{12}C reference system",100,-1,1);
   ...
   TH1F *h_P_miss_1 = new TH1F("P_miss_1","P_{ miss} in ^{12}C reference system",70,0,0.7);
   TH1F *h_P_miss_2 = new TH1F("P_miss_2","P_{ miss} in ^{12}C reference system",70,0,0.7);
   TH1F *h_P_miss = new TH1F("P_miss","P_{ miss} in ^{12}C reference system",70,0,0.7); 
   // TH1F *h_P_miss = new TH1F("P_miss","P_{ miss} in ^{12}C reference system",14,0,0.7); 
   // h_P_miss->SetStats(0);
   ...
   TH1F *h_E_miss_with_tail = new TH1F("E_miss_with_tail","E_{ miss} in ^{12}C reference system",83,-1,1.5);
   TH1F *h_E_miss = new TH1F("E_miss","E_{ miss} in ^{12}C reference system",83,-1,1.5);
   TH1F *h_E_miss_1 = new TH1F("E_miss_1","E_{ miss} in ^{12}C reference system",83,-1,1.5);
   TH1F *h_E_miss_2 = new TH1F("E_miss_2","E_{ miss} in ^{12}C reference system",83,-1,1.5);
   ...
   // TH1F *h_M_miss_square = new TH1F("M_miss_square","M^{2}_{ miss } in ^{12}C reference system",80,-0.64,1.92);
   // TH1F *h_M_miss_square_1 = new TH1F("M_miss_square_1","M^{2}_{ miss } in ^{12}C reference system",80,-0.64,1.92);
   // TH1F *h_M_miss_square_2 = new TH1F("M_miss_square_2","M^{2}_{ miss } in ^{12}C reference system",80,-0.64,1.92);
   // TH1F *h_M_miss_square = new TH1F("M_miss_square","M^{2}_{ miss } in ^{12}C reference system",100,-2,2);
   // TH1F *h_M_miss_square_1 = new TH1F("M_miss_square_1","M^{2}_{ miss } in ^{12}C reference system",100,-2,2);
   // TH1F *h_M_miss_square_2 = new TH1F("M_miss_square_2","M^{2}_{ miss } in ^{12}C reference system",100,-2,2);
   TH1F *h_M_miss_square = new TH1F("M_miss_square","M^{2}_{ miss } in ^{12}C reference system",100,-0.5,3.5);
   TH1F *h_M_miss_square_1 = new TH1F("M_miss_square_1","M^{2}_{ miss } in ^{12}C reference system",100,-0.5,3.5);
   TH1F *h_M_miss_square_2 = new TH1F("M_miss_square_2","M^{2}_{ miss } in ^{12}C reference system",100,-0.5,3.5);
   ...
   TH1F *h_opening_theta = new TH1F("opening_theta","Lab-frame #theta opening angle",60,50,80);//180 90
   TH1F *h_opening_theta_1 = new TH1F("opening_theta_1","Lab-frame #theta opening angle",60,50,80);
   TH1F *h_opening_theta_2 = new TH1F("opening_theta_2","Lab-frame #theta opening angle",60,50,80);
   ...
   TH2F *h_opening_theta_Emiss = new TH2F("opening_theta_Emiss","Lab-frame #theta opening angle vs. Emiss",83,-1,0.5,60,55,75);
   h_opening_theta_Emiss->SetXTitle("Emiss (GeV/c)");
   // h_opening_theta_Emiss->GetXaxis()->SetNdivisions(10,kTRUE);
	h_opening_theta_Emiss->GetXaxis()->CenterTitle(1);
	h_opening_theta_Emiss->GetXaxis()->SetTitleFont(32);
   h_opening_theta_Emiss->GetXaxis()->SetTitleSize(0.06);
	h_opening_theta_Emiss->GetXaxis()->SetTitleOffset(0.7);
   h_opening_theta_Emiss->SetYTitle("#theta_{1} + #theta_{ 2} (degree)");
	h_opening_theta_Emiss->GetYaxis()->CenterTitle(1);
	h_opening_theta_Emiss->GetYaxis()->SetTitleFont(32);
   h_opening_theta_Emiss->GetYaxis()->SetTitleSize(0.04);

   TH2F *h_opening_theta_Mmiss2 = new TH2F("opening_theta_Mmiss2","Lab-frame #theta opening angle vs. Mmiss^{2}",83,-0.,2.,60,55,75);
   h_opening_theta_Mmiss2->SetXTitle("Mmiss^{2} (GeV/c)");
   // h_opening_theta_Mmiss2->GetXaxis()->SetNdivisions(10,kTRUE);
	h_opening_theta_Mmiss2->GetXaxis()->CenterTitle(1);
	h_opening_theta_Mmiss2->GetXaxis()->SetTitleFont(32);
   h_opening_theta_Mmiss2->GetXaxis()->SetTitleSize(0.06);
	h_opening_theta_Mmiss2->GetXaxis()->SetTitleOffset(0.7);
   h_opening_theta_Mmiss2->SetYTitle("#theta_{1} + #theta_{ 2} (degree)");
	h_opening_theta_Mmiss2->GetYaxis()->CenterTitle(1);
	h_opening_theta_Mmiss2->GetYaxis()->SetTitleFont(32);
   h_opening_theta_Mmiss2->GetYaxis()->SetTitleSize(0.04);

   TH1F *h_opening_phi = new TH1F("opening_phi","Lab-frame #phi opening angle",100,-180,180);
   TH1F *h_opening_phi_1 = new TH1F("opening_phi_1","Lab-frame #phi opening angle",100,-180,180);
   TH1F *h_opening_phi_2 = new TH1F("opening_phi_2","Lab-frame #phi opening angle",100,-180,180);
   ...
   TH2F *h_opening_phi_Emiss = new TH2F("opening_phi_Emiss","Lab-frame #phi opening angle vs. Emiss",83,-1,1.5,100,-180,180);
   h_opening_phi_Emiss->SetXTitle("Emiss (GeV/c)");
   // h_opening_phi_Emiss->GetXaxis()->SetNdivisions(10,kTRUE);
	h_opening_phi_Emiss->GetXaxis()->CenterTitle(1);
	h_opening_phi_Emiss->GetXaxis()->SetTitleFont(32);
   h_opening_phi_Emiss->GetXaxis()->SetTitleSize(0.06);
	h_opening_phi_Emiss->GetXaxis()->SetTitleOffset(0.7);
   h_opening_phi_Emiss->SetYTitle("#phi_{1} - #phi_{2} (degree)");
	h_opening_phi_Emiss->GetYaxis()->CenterTitle(1);
	h_opening_phi_Emiss->GetYaxis()->SetTitleFont(32);
   h_opening_phi_Emiss->GetYaxis()->SetTitleSize(0.04);
   ...
   TH1F *h_theta1 = new TH1F("theta1","#theta_{left} distribution",100,0.,90.);
   TH1F *h_theta1_1 = new TH1F("theta1_1","#theta_{left} distribution",100,0.,90.);
   ...
   TH1F *h_theta2 = new TH1F("theta2","#theta_{right} distribution",100,0.,90);
   TH1F *h_theta2_1 = new TH1F("theta2_1","#theta_{right} distribution",100,0.,90);
   ...
   TH2F *h_theta12 = new TH2F("theta12","#theta_{1} vs. #theta_{2}",60,0.,60.,60,0.,60);
   h_theta12->SetXTitle("#theta_{1} (degree)");
   // h_theta12->GetXaxis()->SetNdivisions(10,kTRUE);
	h_theta12->GetXaxis()->CenterTitle(1);
	h_theta12->GetXaxis()->SetTitleFont(32);
   h_theta12->GetXaxis()->SetTitleSize(0.06);
	h_theta12->GetXaxis()->SetTitleOffset(0.7);
   h_theta12->SetYTitle("#theta_{2} (degree)");
	h_theta12->GetYaxis()->CenterTitle(1);
	h_theta12->GetYaxis()->SetTitleFont(32);
   h_theta12->GetYaxis()->SetTitleSize(0.04);

   TH1F *h_phi1_1 = new TH1F("phi1_1","#phi_{left} distribution",50,-90.,90.);
   TH1F *h_phi1 = new TH1F("phi1","#phi_{left} distribution",50,-90.,90.);
   ...
   TH1F *h_phi2_1 = new TH1F("phi2_1","#phi_{right} distribution",100,-180.,180.);
   TH1F *h_phi2 = new TH1F("phi2","#phi_{right} distribution",100,-180.,180.);
   ...
   TH2F *h_phi12 = new TH2F("phi1_phi2","lab-frame #phi_{1} vs. #phi_{2}",100,-90,90,100,-180,180);
   ...
   TH1F *h_vertex_x = new TH1F("vertex_x","X vertex position",100,-4,6);
   ...
   TH1F *h_vertex_y = new TH1F("vertex_y","Y vertex position",100,-5,5);
   h_vertex_y->SetXTitle("Y / cm");
   ...
   // TH1F *h_vertex_z = new TH1F("vertex_z","Z vertex position",100,-600,-550);
   // TH1F *h_vertex_z_1 = new TH1F("vertex_z_1","Z vertex position",65,-624,-533);
   // TH1F *h_vertex_z = new TH1F("vertex_z","Z vertex position",65,-624,-533);
   TH1F *h_vertex_z_1 = new TH1F("vertex_z_1","Z vertex position",40,-603,-547); // -650 -510
   TH1F *h_vertex_z_breakup = new TH1F("vertex_z_breakup","Z vertex position",40,-603,-547);
   TH1F *h_vertex_z = new TH1F("vertex_z","Z vertex position",40,-603,-547);
   ...
   TH1F *h_vertex_z_with_cut = new TH1F("vertex_z_with_cut","Z vertex position with cut around #Theta* of 90^{o}",65,-624,-533);
   ...
   TH1F *h_p_miss_diff_over_pmiss = new TH1F("p_miss_diff_over_pmiss","#Delta P_{miss} / P_{miss} in ^{12}C reference system",200,-1,1);
   h_p_miss_diff_over_pmiss->SetXTitle("#Delta P_{miss} / P_{miss}");
	h_p_miss_diff_over_pmiss->GetXaxis()->CenterTitle(1);
	h_p_miss_diff_over_pmiss->GetXaxis()->SetTitleFont(32);
   h_p_miss_diff_over_pmiss->GetXaxis()->SetTitleSize(0.06);
	h_p_miss_diff_over_pmiss->GetXaxis()->SetTitleOffset(0.7);
   h_p_miss_diff_over_pmiss->SetYTitle("Counts");
	h_p_miss_diff_over_pmiss->GetYaxis()->CenterTitle(1);
	h_p_miss_diff_over_pmiss->GetYaxis()->SetTitleFont(32);
   h_p_miss_diff_over_pmiss->GetYaxis()->SetTitleSize(0.04);
   ...
   TH2F* h_Pmiss_Pfrag_z = new TH2F("h_Pmiss_Pfrag_z","Pmiss_{z} vs. Pfrag_{z} in ^{12}C reference system",100,-0.5,0.5,100,-0.5,0.5);
   ...
   TH1F* h_pbx_1 = new TH1F("h_pbx_1","^{11}B momentum distribution in ^{12}C reference system",100,-1,1);
   TH1F* h_pbx_2 = new TH1F("h_pbx_2","^{11}B momentum distribution in ^{12}C reference system",100,-1,1);
   TH1F* h_pbx = new TH1F("h_pbx","^{11}B momentum distribution in ^{12}C reference system",100,-1,1);
   ...
   TH1F* h_pby_1 = new TH1F("h_pby_1","^{11}B momentum distribution in ^{12}C reference system",100,-1,1);
   TH1F* h_pby_2 = new TH1F("h_pby_2","^{11}B momentum distribution in ^{12}C reference system",100,-1,1);
   TH1F* h_pby = new TH1F("h_pby","^{11}B momentum distribution in ^{12}C reference system",100,-1,1);
   ...
   TH1F* h_pbz_1 = new TH1F("h_pbz_1","^{11}B momentum distribution in ^{12}C reference system",100,-1,1);
   TH1F* h_pbz_2 = new TH1F("h_pbz_2","^{11}B momentum distribution in ^{12}C reference system",100,-1,1);
   TH1F* h_pbz = new TH1F("h_pbz","^{11}B momentum distribution in ^{12}C reference system",100,-1,1);
   ...
   TH1F* h_pb_1 = new TH1F("h_pb_1","^{11}B momentum distribution in ^{12}C reference system",100,0,1);
   TH1F* h_pb_2 = new TH1F("h_pb_2","^{11}B momentum distribution in ^{12}C reference system",100,0,1);
   TH1F* h_pb = new TH1F("h_pb","^{11}B momentum distribution in ^{12}C reference system",100,0,1);
   ...
   TH1F* h_fly_path_left = new TH1F("h_fly_path_left","Fly Length to TOF400 (Left)",100,200,300);
   ...
   TH1F* h_fly_path_right = new TH1F("h_fly_path_right","Fly Length to TOF400 (Right)",100,200,300);
   ...
   TH1F* h_fly_time_left = new TH1F("h_fly_time_left","Fly Time to TOF400 (Left)",100,0,20);
   h_fly_time_left->SetXTitle("Time / ns");
   ...
   TH1F* h_fly_time_right = new TH1F("h_fly_time_right","Fly Time to TOF400 (Right)",100,0,20);
   h_fly_time_right->SetXTitle("Time / ns");
   ...
   // 38
   TH1F *h_M_miss_square_frag_highstat_with_accp = new TH1F("M_miss_square_frag_highstat_with_accp","M^{2}_{ miss }",60,0.08,2);
   TH1F *h_M_miss_square_frag_highstat = new TH1F("M_miss_square_frag_highstat","M^{2}_{ miss }",60,0.08,2);
   h_M_miss_square_frag_highstat_with_accp->SetXTitle("M^{2}_{ miss } / GeV");
	h_M_miss_square_frag_highstat_with_accp->GetXaxis()->CenterTitle(1);
	h_M_miss_square_frag_highstat_with_accp->GetXaxis()->SetTitleFont(32);
   h_M_miss_square_frag_highstat_with_accp->GetXaxis()->SetTitleSize(0.06);
	h_M_miss_square_frag_highstat_with_accp->GetXaxis()->SetTitleOffset(0.7);
   h_M_miss_square_frag_highstat_with_accp->SetYTitle("Counts");
	h_M_miss_square_frag_highstat_with_accp->GetYaxis()->CenterTitle(1);
	h_M_miss_square_frag_highstat_with_accp->GetYaxis()->SetTitleFont(32);
   h_M_miss_square_frag_highstat_with_accp->GetYaxis()->SetTitleSize(0.04);

   TH1F *h_P_miss_frag_highstat_with_accp = new TH1F("P_miss_frag_highstat_with_accp","P_{ miss }",70,0,0.7);
   TH1F *h_P_miss_frag_highstat = new TH1F("P_miss_frag_highstat","P_{ miss }",70,0,0.7);
   h_P_miss_frag_highstat_with_accp->SetXTitle("P_{ miss } / GeV");
	h_P_miss_frag_highstat_with_accp->GetXaxis()->CenterTitle(1);
	h_P_miss_frag_highstat_with_accp->GetXaxis()->SetTitleFont(32);
   h_P_miss_frag_highstat_with_accp->GetXaxis()->SetTitleSize(0.06);
	h_P_miss_frag_highstat_with_accp->GetXaxis()->SetTitleOffset(0.7);
   h_P_miss_frag_highstat_with_accp->SetYTitle("Counts");
	h_P_miss_frag_highstat_with_accp->GetYaxis()->CenterTitle(1);
	h_P_miss_frag_highstat_with_accp->GetYaxis()->SetTitleFont(32);
   h_P_miss_frag_highstat_with_accp->GetYaxis()->SetTitleSize(0.04);

   TH1F *h_P_miss_x_frag_highstat_with_accp = new TH1F("P_miss_x_frag_highstat_with_accp","Pmiss_{ x }",100,-1,1);
   TH1F *h_P_miss_x_frag_highstat = new TH1F("P_miss_x_frag_highstat","Pmiss_{ x }",100,-1,1);
   h_P_miss_x_frag_highstat_with_accp->SetXTitle("Pmiss_{ x } / GeV");
	h_P_miss_x_frag_highstat_with_accp->GetXaxis()->CenterTitle(1);
	h_P_miss_x_frag_highstat_with_accp->GetXaxis()->SetTitleFont(32);
   h_P_miss_x_frag_highstat_with_accp->GetXaxis()->SetTitleSize(0.06);
	h_P_miss_x_frag_highstat_with_accp->GetXaxis()->SetTitleOffset(0.7);
   h_P_miss_x_frag_highstat_with_accp->SetYTitle("Counts");
	h_P_miss_x_frag_highstat_with_accp->GetYaxis()->CenterTitle(1);
	h_P_miss_x_frag_highstat_with_accp->GetYaxis()->SetTitleFont(32);
   h_P_miss_x_frag_highstat_with_accp->GetYaxis()->SetTitleSize(0.04);

   TH1F *h_P_miss_y_frag_highstat_with_accp = new TH1F("P_miss_y_frag_highstat_with_accp","Pmiss_{ y }",100,-1,1);
   TH1F *h_P_miss_y_frag_highstat = new TH1F("P_miss_y_frag_highstat","Pmiss_{ y }",100,-1,1);
   h_P_miss_y_frag_highstat_with_accp->SetXTitle("Pmiss_{ y }/ GeV");
	h_P_miss_y_frag_highstat_with_accp->GetXaxis()->CenterTitle(1);
	h_P_miss_y_frag_highstat_with_accp->GetXaxis()->SetTitleFont(32);
   h_P_miss_y_frag_highstat_with_accp->GetXaxis()->SetTitleSize(0.06);
	h_P_miss_y_frag_highstat_with_accp->GetXaxis()->SetTitleOffset(0.7);
   h_P_miss_y_frag_highstat_with_accp->SetYTitle("Counts");
	h_P_miss_y_frag_highstat_with_accp->GetYaxis()->CenterTitle(1);
	h_P_miss_y_frag_highstat_with_accp->GetYaxis()->SetTitleFont(32);
   h_P_miss_y_frag_highstat_with_accp->GetYaxis()->SetTitleSize(0.04);

   TH1F *h_P_miss_z_frag_highstat_with_accp = new TH1F("P_miss_z_frag_highstat_with_accp","Pmiss_{ z }",100,-1,1);
   TH1F *h_P_miss_z_frag_highstat = new TH1F("P_miss_z_frag_highstat","Pmiss_{ z }",100,-1,1);
   h_P_miss_z_frag_highstat_with_accp->SetXTitle("Pmiss_{ z }/ GeV");
	h_P_miss_z_frag_highstat_with_accp->GetXaxis()->CenterTitle(1);
	h_P_miss_z_frag_highstat_with_accp->GetXaxis()->SetTitleFont(32);
   h_P_miss_z_frag_highstat_with_accp->GetXaxis()->SetTitleSize(0.06);
	h_P_miss_z_frag_highstat_with_accp->GetXaxis()->SetTitleOffset(0.7);
   h_P_miss_z_frag_highstat_with_accp->SetYTitle("Counts");
	h_P_miss_z_frag_highstat_with_accp->GetYaxis()->CenterTitle(1);
	h_P_miss_z_frag_highstat_with_accp->GetYaxis()->SetTitleFont(32);
   h_P_miss_z_frag_highstat_with_accp->GetYaxis()->SetTitleSize(0.04);

   // 39
   TH1F *h_M_miss_square_frag_highstat_with_accp_diff = new TH1F("M_miss_square_frag_highstat_with_accp_diff","M^{2}_{ miss } difference between two reconstruction methods",100,-1,1);
   h_M_miss_square_frag_highstat_with_accp_diff->SetXTitle("M^{2}_{ miss } / GeV");
	h_M_miss_square_frag_highstat_with_accp_diff->GetXaxis()->CenterTitle(1);
	h_M_miss_square_frag_highstat_with_accp_diff->GetXaxis()->SetTitleFont(32);
   h_M_miss_square_frag_highstat_with_accp_diff->GetXaxis()->SetTitleSize(0.06);
	h_M_miss_square_frag_highstat_with_accp_diff->GetXaxis()->SetTitleOffset(0.7);
   h_M_miss_square_frag_highstat_with_accp_diff->SetYTitle("Counts");
	h_M_miss_square_frag_highstat_with_accp_diff->GetYaxis()->CenterTitle(1);
	h_M_miss_square_frag_highstat_with_accp_diff->GetYaxis()->SetTitleFont(32);
   h_M_miss_square_frag_highstat_with_accp_diff->GetYaxis()->SetTitleSize(0.04);

   TH1F *h_P_miss_frag_highstat_with_accp_diff = new TH1F("P_miss_frag_highstat_with_accp_diff","P_{ miss } difference between two reconstruction methods",100,-1,1);
   h_P_miss_frag_highstat_with_accp_diff->SetXTitle("P_{ miss } / GeV");
	h_P_miss_frag_highstat_with_accp_diff->GetXaxis()->CenterTitle(1);
	h_P_miss_frag_highstat_with_accp_diff->GetXaxis()->SetTitleFont(32);
   h_P_miss_frag_highstat_with_accp_diff->GetXaxis()->SetTitleSize(0.06);
	h_P_miss_frag_highstat_with_accp_diff->GetXaxis()->SetTitleOffset(0.7);
   h_P_miss_frag_highstat_with_accp_diff->SetYTitle("Counts");
	h_P_miss_frag_highstat_with_accp_diff->GetYaxis()->CenterTitle(1);
	h_P_miss_frag_highstat_with_accp_diff->GetYaxis()->SetTitleFont(32);
   h_P_miss_frag_highstat_with_accp_diff->GetYaxis()->SetTitleSize(0.04);

   TH1F *h_P_miss_x_frag_highstat_with_accp_diff = new TH1F("P_miss_x_frag_highstat_with_accp_diff","Pmiss_{ x } difference between two reconstruction methods",100,-1,1);
   h_P_miss_x_frag_highstat_with_accp_diff->SetXTitle("Pmiss_{ x } / GeV");
	h_P_miss_x_frag_highstat_with_accp_diff->GetXaxis()->CenterTitle(1);
	h_P_miss_x_frag_highstat_with_accp_diff->GetXaxis()->SetTitleFont(32);
   h_P_miss_x_frag_highstat_with_accp_diff->GetXaxis()->SetTitleSize(0.06);
	h_P_miss_x_frag_highstat_with_accp_diff->GetXaxis()->SetTitleOffset(0.7);
   h_P_miss_x_frag_highstat_with_accp_diff->SetYTitle("Counts");
	h_P_miss_x_frag_highstat_with_accp_diff->GetYaxis()->CenterTitle(1);
	h_P_miss_x_frag_highstat_with_accp_diff->GetYaxis()->SetTitleFont(32);
   h_P_miss_x_frag_highstat_with_accp_diff->GetYaxis()->SetTitleSize(0.04);

   TH1F *h_P_miss_y_frag_highstat_with_accp_diff = new TH1F("P_miss_y_frag_highstat_with_accp_diff","Pmiss_{ y } difference between two reconstruction methods",100,-1,1);
   h_P_miss_y_frag_highstat_with_accp_diff->SetXTitle("Pmiss_{ y }/ GeV");
	h_P_miss_y_frag_highstat_with_accp_diff->GetXaxis()->CenterTitle(1);
	h_P_miss_y_frag_highstat_with_accp_diff->GetXaxis()->SetTitleFont(32);
   h_P_miss_y_frag_highstat_with_accp_diff->GetXaxis()->SetTitleSize(0.06);
	h_P_miss_y_frag_highstat_with_accp_diff->GetXaxis()->SetTitleOffset(0.7);
   h_P_miss_y_frag_highstat_with_accp_diff->SetYTitle("Counts");
	h_P_miss_y_frag_highstat_with_accp_diff->GetYaxis()->CenterTitle(1);
	h_P_miss_y_frag_highstat_with_accp_diff->GetYaxis()->SetTitleFont(32);
   h_P_miss_y_frag_highstat_with_accp_diff->GetYaxis()->SetTitleSize(0.04);

   TH1F *h_P_miss_z_frag_highstat_with_accp_diff = new TH1F("P_miss_z_frag_highstat_with_accp_diff","Pmiss_{ z } difference between two reconstruction methods",100,-1,1);
   h_P_miss_z_frag_highstat_with_accp_diff->SetXTitle("Pmiss_{ z }/ GeV");
	h_P_miss_z_frag_highstat_with_accp_diff->GetXaxis()->CenterTitle(1);
	h_P_miss_z_frag_highstat_with_accp_diff->GetXaxis()->SetTitleFont(32);
   h_P_miss_z_frag_highstat_with_accp_diff->GetXaxis()->SetTitleSize(0.06);
	h_P_miss_z_frag_highstat_with_accp_diff->GetXaxis()->SetTitleOffset(0.7);
   h_P_miss_z_frag_highstat_with_accp_diff->SetYTitle("Counts");
	h_P_miss_z_frag_highstat_with_accp_diff->GetYaxis()->CenterTitle(1);
	h_P_miss_z_frag_highstat_with_accp_diff->GetYaxis()->SetTitleFont(32);
   h_P_miss_z_frag_highstat_with_accp_diff->GetYaxis()->SetTitleSize(0.04);

   // 40
   TH2F *h_M_miss_square_frag_highstat_with_accp_vs_method1 = new TH2F("M_miss_square_frag_highstat_with_accp_vs_method1","M^{2}_{ miss } from two reconstruction methods",100,0,2,100,0,2);
   h_M_miss_square_frag_highstat_with_accp_vs_method1->SetXTitle("M^{2}_{ miss } from Fragments / GeV");
	h_M_miss_square_frag_highstat_with_accp_vs_method1->GetXaxis()->CenterTitle(1);
	h_M_miss_square_frag_highstat_with_accp_vs_method1->GetXaxis()->SetTitleFont(32);
   h_M_miss_square_frag_highstat_with_accp_vs_method1->GetXaxis()->SetTitleSize(0.06);
	h_M_miss_square_frag_highstat_with_accp_vs_method1->GetXaxis()->SetTitleOffset(0.7);
   h_M_miss_square_frag_highstat_with_accp_vs_method1->SetYTitle("M^{2}_{ miss } from Protons / GeV");
	h_M_miss_square_frag_highstat_with_accp_vs_method1->GetYaxis()->CenterTitle(1);
	h_M_miss_square_frag_highstat_with_accp_vs_method1->GetYaxis()->SetTitleFont(32);
   h_M_miss_square_frag_highstat_with_accp_vs_method1->GetYaxis()->SetTitleSize(0.04);

   TH2F *h_P_miss_frag_highstat_with_accp_vs_method1 = new TH2F("P_miss_frag_highstat_with_accp_vs_method1","P_{ miss } from two reconstruction methods",70,0,0.7,70,0,0.7);
   h_P_miss_frag_highstat_with_accp_vs_method1->SetYTitle("P_{ miss } from Fragments / GeV");
   h_P_miss_frag_highstat_with_accp_vs_method1->SetXTitle("P_{ miss } from Protons / GeV");
	h_P_miss_frag_highstat_with_accp_vs_method1->GetXaxis()->CenterTitle(1);
	h_P_miss_frag_highstat_with_accp_vs_method1->GetXaxis()->SetTitleFont(32);
   h_P_miss_frag_highstat_with_accp_vs_method1->GetXaxis()->SetTitleSize(0.06);
	h_P_miss_frag_highstat_with_accp_vs_method1->GetXaxis()->SetTitleOffset(0.7);
	h_P_miss_frag_highstat_with_accp_vs_method1->GetYaxis()->CenterTitle(1);
	h_P_miss_frag_highstat_with_accp_vs_method1->GetYaxis()->SetTitleFont(32);
   h_P_miss_frag_highstat_with_accp_vs_method1->GetYaxis()->SetTitleSize(0.04);

   TH2F *h_P_miss_x_frag_highstat_with_accp_vs_method1 = new TH2F("P_miss_x_frag_highstat_with_accp_vs_method1","Pmiss_{ x } from two reconstruction methods",100,-1,1,100,-1,1);
   h_P_miss_x_frag_highstat_with_accp_vs_method1->SetYTitle("Pmiss_{ x } from Fragments / GeV");
   h_P_miss_x_frag_highstat_with_accp_vs_method1->SetXTitle("Pmiss_{ x } from Protons / GeV");
	h_P_miss_x_frag_highstat_with_accp_vs_method1->GetXaxis()->CenterTitle(1);
	h_P_miss_x_frag_highstat_with_accp_vs_method1->GetXaxis()->SetTitleFont(32);
   h_P_miss_x_frag_highstat_with_accp_vs_method1->GetXaxis()->SetTitleSize(0.06);
	h_P_miss_x_frag_highstat_with_accp_vs_method1->GetXaxis()->SetTitleOffset(0.7);
	h_P_miss_x_frag_highstat_with_accp_vs_method1->GetYaxis()->CenterTitle(1);
	h_P_miss_x_frag_highstat_with_accp_vs_method1->GetYaxis()->SetTitleFont(32);
   h_P_miss_x_frag_highstat_with_accp_vs_method1->GetYaxis()->SetTitleSize(0.04);

   TH2F *h_P_miss_y_frag_highstat_with_accp_vs_method1 = new TH2F("P_miss_y_frag_highstat_with_accp_vs_method1","Pmiss_{ y } from two reconstruction methods",100,-1,1,100,-1,1);
   h_P_miss_y_frag_highstat_with_accp_vs_method1->SetYTitle("Pmiss_{ y } from Fragments / GeV");
   h_P_miss_y_frag_highstat_with_accp_vs_method1->SetXTitle("Pmiss_{ y } from Protons / GeV");
	h_P_miss_y_frag_highstat_with_accp_vs_method1->GetXaxis()->CenterTitle(1);
	h_P_miss_y_frag_highstat_with_accp_vs_method1->GetXaxis()->SetTitleFont(32);
   h_P_miss_y_frag_highstat_with_accp_vs_method1->GetXaxis()->SetTitleSize(0.06);
	h_P_miss_y_frag_highstat_with_accp_vs_method1->GetXaxis()->SetTitleOffset(0.7);
	h_P_miss_y_frag_highstat_with_accp_vs_method1->GetYaxis()->CenterTitle(1);
	h_P_miss_y_frag_highstat_with_accp_vs_method1->GetYaxis()->SetTitleFont(32);
   h_P_miss_y_frag_highstat_with_accp_vs_method1->GetYaxis()->SetTitleSize(0.04);

   TH2F *h_P_miss_z_frag_highstat_with_accp_vs_method1 = new TH2F("P_miss_z_frag_highstat_with_accp_vs_method1","Pmiss_{ z } from two reconstruction methods",100,-1,1,100,-1,1);
   h_P_miss_z_frag_highstat_with_accp_vs_method1->SetYTitle("Pmiss_{ z } from Fragments / GeV");
   h_P_miss_z_frag_highstat_with_accp_vs_method1->SetXTitle("Pmiss_{ z } from Protons / GeV");
	h_P_miss_z_frag_highstat_with_accp_vs_method1->GetXaxis()->CenterTitle(1);
	h_P_miss_z_frag_highstat_with_accp_vs_method1->GetXaxis()->SetTitleFont(32);
   h_P_miss_z_frag_highstat_with_accp_vs_method1->GetXaxis()->SetTitleSize(0.06);
	h_P_miss_z_frag_highstat_with_accp_vs_method1->GetXaxis()->SetTitleOffset(0.7);
	h_P_miss_z_frag_highstat_with_accp_vs_method1->GetYaxis()->CenterTitle(1);
	h_P_miss_z_frag_highstat_with_accp_vs_method1->GetYaxis()->SetTitleFont(32);
   h_P_miss_z_frag_highstat_with_accp_vs_method1->GetYaxis()->SetTitleSize(0.04);
   ...
   TH2F* h_Pmiss_diff_ratio_frag_highstat = new TH2F("h_Pmiss_diff_ratio_frag_highstat","(Pmiss_{ frag } - Pmiss_{ arm }) / Pmiss_{ frag } vs. Pmiss_{ frag }",200,0,2,300,-200,100);
   h_Pmiss_diff_ratio_frag_highstat->SetXTitle("Pmiss_{ frag } / GeV");
	h_Pmiss_diff_ratio_frag_highstat->GetXaxis()->CenterTitle(1);
	h_Pmiss_diff_ratio_frag_highstat->GetXaxis()->SetTitleFont(32);
   h_Pmiss_diff_ratio_frag_highstat->GetXaxis()->SetTitleSize(0.06);
	h_Pmiss_diff_ratio_frag_highstat->GetXaxis()->SetTitleOffset(0.7);
   h_Pmiss_diff_ratio_frag_highstat->SetYTitle("Percentage");
	h_Pmiss_diff_ratio_frag_highstat->GetYaxis()->CenterTitle(1);
	h_Pmiss_diff_ratio_frag_highstat->GetYaxis()->SetTitleFont(32);
   h_Pmiss_diff_ratio_frag_highstat->GetYaxis()->SetTitleSize(0.04);
   ...
   TH2F* h_Pmiss_frag_highstat_theta = new TH2F("h_Pmiss_frag_highstat_theta","Pmiss_{ frag } vs. Theta",200,0,100,200,0,2);
   h_Pmiss_frag_highstat_theta->SetXTitle("Theta");
	h_Pmiss_frag_highstat_theta->GetXaxis()->CenterTitle(1);
	h_Pmiss_frag_highstat_theta->GetXaxis()->SetTitleFont(32);
   h_Pmiss_frag_highstat_theta->GetXaxis()->SetTitleSize(0.06);
	h_Pmiss_frag_highstat_theta->GetXaxis()->SetTitleOffset(0.7);
   h_Pmiss_frag_highstat_theta->SetYTitle("Pmiss_{ frag } / GeV");
	h_Pmiss_frag_highstat_theta->GetYaxis()->CenterTitle(1);
	h_Pmiss_frag_highstat_theta->GetYaxis()->SetTitleFont(32);
   h_Pmiss_frag_highstat_theta->GetYaxis()->SetTitleSize(0.04);
   ...
   TH1F *h_Trieman_Yang_Angle = new TH1F("Trieman_Yang_Angle","Trieman Yang Angle",100,-1,1);
   h_Trieman_Yang_Angle->SetXTitle("cosine of the angle");
	h_Trieman_Yang_Angle->GetXaxis()->CenterTitle(1);
	h_Trieman_Yang_Angle->GetXaxis()->SetTitleFont(32);
   h_Trieman_Yang_Angle->GetXaxis()->SetTitleSize(0.06);
	h_Trieman_Yang_Angle->GetXaxis()->SetTitleOffset(0.7);
   h_Trieman_Yang_Angle->SetYTitle("Counts");
	h_Trieman_Yang_Angle->GetYaxis()->CenterTitle(1);
	h_Trieman_Yang_Angle->GetYaxis()->SetTitleFont(32);
   h_Trieman_Yang_Angle->GetYaxis()->SetTitleSize(0.04);

   TFile *file_output_QE = new TFile("./QE_Simulation_delta_test_frag_highstat.root","RECREATE"); // "UPDATE"
   ...
   tree_QE->Branch("weight",&weight_QE,"weight/D");
   ...
   tree_QE->Branch("Hit_TOF_left",Hit_TOF_left,"Hit_TOF_left[3]/D");
   tree_QE->Branch("Hit_TOF_right",Hit_TOF_right,"Hit_TOF_right[3]/D");
   ...
   Double_t NormFactor = 452; // No beta cut
   ...
   // for (Long64_t jentry=0; jentry<100;jentry++) {
      ...
      // PBX = gRandom->Gaus(PBx/1000.,PBx*PB_resolution_xy/1000.);
      // PBY = gRandom->Gaus(PBy/1000.,PBy*PB_resolution_xy/1000.);
      PBX = gRandom->Gaus(PBx/1000.,PB_resolution_xy);
      PBY = gRandom->Gaus(PBy/1000.,PB_resolution_xy);
      PBZ = gRandom->Gaus(PBz/1000.,PBz*PB_resolution/1000.);

      // Double_t P_12C_frag_highstat = gRandom->Gaus(P_12C_Real,P_12C_Real*0.01);
      Double_t P_12C_frag_highstat = P_12C_Real;
      Double_t beta_12C_frag_highstat = P_12C_frag_highstat/m_12C/sqrt(1.+pow(P_12C_frag_highstat/m_12C,2));
      TLorentzRotation boost_frag_highstat;
      boost_frag_highstat.Boost(0,0,-beta_12C_frag_highstat);

      TLorentzVector Frag, Beam, proton_miss, proton_miss_initial;
      Frag.SetXYZM(PBX,PBY,PBZ,m_11B);
      Beam.SetXYZM(0.,0.,P_12C_frag_highstat,m_12C);
      proton_miss_initial = Beam - Frag;
      proton_miss = boost_frag_highstat *proton_miss_initial;
      Double_t M2_frag_highstat = proton_miss.M2();
      Double_t Pmiss_x_frag_highstat = proton_miss.Px();
      Double_t Pmiss_y_frag_highstat = proton_miss.Py();
      Double_t Pmiss_z_frag_highstat = proton_miss.Pz();
      Double_t Pmiss_frag_highstat = sqrt(pow(Pmiss_x_frag_highstat,2)+pow(Pmiss_y_frag_highstat,2)+pow(Pmiss_z_frag_highstat,2));
      h_M_miss_square_frag_highstat->Fill(M2_frag_highstat);
      h_P_miss_frag_highstat->Fill(Pmiss_frag_highstat);
      h_P_miss_x_frag_highstat->Fill(Pmiss_x_frag_highstat);
      h_P_miss_y_frag_highstat->Fill(Pmiss_y_frag_highstat);
      h_P_miss_z_frag_highstat->Fill(Pmiss_z_frag_highstat);
      ...
      kp = kp1 + kp2;
      h_Pfragx_PmissTheta_all->Fill(kp.Angle(kz)/3.1415926535*180,PBX);
      h_Pfragx_OpeningTheta_all->Fill(kp1.Angle(kp2)/3.1415926535*180,PBX);
      h_Pfragy_PmissTheta_all->Fill(kp.Angle(kz)/3.1415926535*180,PBY);
      h_Pfragy_OpeningTheta_all->Fill(kp1.Angle(kp2)/3.1415926535*180,PBY);
      h_Pfragz_PmissTheta_all->Fill(kp.Angle(kz)/3.1415926535*180,PBZ);
      h_Pfragz_OpeningTheta_all->Fill(kp1.Angle(kp2)/3.1415926535*180,PBZ);
      ...
      h_pbx_2->Fill(PB_transformed.Px());
      h_pby_2->Fill(PB_transformed.Py());
      h_pbz_2->Fill(PB_transformed.Pz());
      h_pb_2->Fill(sqrt(pow(PB_transformed.Px(),2)+pow(PB_transformed.Py(),2)+pow(PB_transformed.Pz(),2)));      
      ...
      // h_Pfragz_theta_left->Fill(theta1_1,PBZ);
      ...
      // h_px_diff_polar->Fill(theta1_x,px1_diff); h_px_diff_polar->Fill(theta2_x,px2_diff);
      // h_py_diff_polar->Fill(theta1_x,py1_diff); h_py_diff_polar->Fill(theta2_x,py2_diff);
      // h_pz_diff_polar->Fill(theta1_x,pz1_diff); h_pz_diff_polar->Fill(theta2_x,pz2_diff);
      // h_P_miss_x_diff_polar->Fill(theta1_x+theta2_x,P_miss_x_diff);
      // h_P_miss_y_diff_polar->Fill(theta1_x+theta2_x,P_miss_y_diff);
      // h_P_miss_z_diff_polar->Fill(theta1_x+theta2_x,P_miss_z_diff);

      // h_px_diff_polar->Fill(theta1_y,px1_diff); h_px_diff_polar->Fill(theta2_y,px2_diff);
      // h_py_diff_polar->Fill(theta1_y,py1_diff); h_py_diff_polar->Fill(theta2_y,py2_diff);
      // h_pz_diff_polar->Fill(theta1_y,pz1_diff); h_pz_diff_polar->Fill(theta2_y,pz2_diff);
      // h_P_miss_x_diff_polar->Fill(theta1_y+theta2_y,P_miss_x_diff);
      // h_P_miss_y_diff_polar->Fill(theta1_y+theta2_y,P_miss_y_diff);
      // h_P_miss_z_diff_polar->Fill(theta1_y+theta2_y,P_miss_z_diff);

      // h_px_diff_polar->Fill(theta1_x+theta2_x,P_miss_x_diff);
      // h_px_diff_polar->Fill(theta1_x+theta2_x,P_miss_y_diff);
      ...
      h_scattering_angle_2->Fill(CA_1);
      double max_t_u_1 = std::max(fabs(tt_1),fabs(uu_1));
         ...
         opening_theta_1 = theta1_1+theta2_1;
      ...
      h_p1xp2x_2->Fill(P_miss_x_1);
      h_p1yp2y_2->Fill(P_miss_y_1);
      h_p1zp2z_2->Fill(P_miss_z_1);
      h_P_miss_2->Fill(P_miss_1);
      h_E_miss_2->Fill(E_miss_1);
      h_M_miss_2->Fill(M_miss_1);
      h_M_miss_square_2->Fill(M_miss_1*M_miss_1);
      h_opening_theta_2->Fill(theta1_1+theta2_1);
      if(phi1_1-phi2_1>-180 && phi1_1-phi2_1<180) h_opening_phi_2->Fill(phi1_1-phi2_1);
      else if(phi1_1-phi2_1<-180) h_opening_phi_2->Fill(phi1_1-phi2_1+360);
      else h_opening_phi_2->Fill(phi1_1-phi2_1-360);
      h_E_theta_all->Fill(E_miss_1,theta1_1+theta2_1);

      NEvents_total++;

      if(CA_1>70.&&CA_1<110.){
         // NEvents_total++;
         // if(max_t_u_1>0&&max_t_u_1<2.9) NEvents_total++;
         // if(max_t_u_1>2.9&&max_t_u_1<3.4) NEvents_total++;
         // if(max_t_u_1>3.4&&max_t_u_1<4) NEvents_total++;
         // if(max_t_u_1>4) NEvents_total++;

         // if(max_t_u_1>2.0&&max_t_u_1<2.5) NEvents_total++;
         // if(max_t_u_1>2.5&&max_t_u_1<3.0) NEvents_total++;
         // if(max_t_u_1>3.0&&max_t_u_1<3.5) NEvents_total++;
         // if(max_t_u_1>3.5&&max_t_u_1<4.0) NEvents_total++;
         // if(max_t_u_1>4.0&&max_t_u_1<4.5) NEvents_total++;
         // if(max_t_u_1>4.5&&max_t_u_1<5.0) NEvents_total++;
         // if(max_t_u_1>5.0&&max_t_u_1<6) NEvents_total++;
      }
      if(CA_1>60.&&CA_1<120.){
         h_theta12_all->Fill(theta1_1,theta2_1);
         h_phi12_all->Fill(phi1_1,phi2_1);
         h_theta_phi_all->Fill(theta1_1,phi1_1);
         h_theta_phi1_all->Fill(theta1_1,phi1_1);
         h_theta_phi_all->Fill(theta2_1,phi2_1);
         h_theta_phi2_all->Fill(theta2_1,phi2_1);
      }
      ...
      // if(vertex_z<-576.2-15||vertex_z>-576.2+15){
      //    // cout<<vertex_z<<endl;
      //    Trigger_GEM=0
      //    Trigger_CSC=0
      //    Trigger_TOF=0
      ...
      // cout<<StsPoint_fPx[0]<<endl;

      int Nofparticles_left = 0;
      int Nofparticles_right = 0;
      int delta_pion_flag = 0;
      int pdg_test = 0;
      for(int i=0;i<kMaxbmnroot_0_TOF1_TOF400Point;i++){
         if(TOF400Point_fTrackID[i]<0) break;
         if(TOF400Point_fTrackID[i]==CSCPoint_fTrackID[i]){
            if(CSCPoint_fX[i]>0) Nofparticles_left++;
            if(CSCPoint_fX[i]<0) Nofparticles_right++;
            if(CSCPoint_fPdgId[i]!=2212){
               delta_pion_flag = 1;
               if(CSCPoint_fPdgId[i]!=111&&CSCPoint_fPdgId[i]!=211&&CSCPoint_fPdgId[i]!=-211) pdg_test = CSCPoint_fPdgId[i];
            }
            // if(CSCPoint_fPdgId[i]==111||CSCPoint_fPdgId[i]==211||CSCPoint_fPdgId[i]==-211) delta_pion_flag = 1;
            // cout<<jentry<<"\t"<<TOF400Point_fTrackID[i]<<"\t"<<TOF400Point_fX[i]<<"\t"<<TOF400Point_fPx[i]<<endl;
            // cout<<jentry<<"\t"<<CSCPoint_fTrackID[i]<<"\t"<<CSCPoint_fX[i]<<"\t"<<CSCPoint_fPx[i]<<"\t"<<CSCPoint_fPdgId[i]<<endl;
         }
      }

      // if(Nofparticles_left>0&&Nofparticles_right>0){
      //    cout<<Nofparticles_left<<"\t"<<Nofparticles_right<<endl;
      //    for(int i=0;i<kMaxbmnroot_0_TOF1_TOF400Point;i++){
      //       if(TOF400Point_fTrackID[i]<0) break;
      //       if(TOF400Point_fTrackID[i]==CSCPoint_fTrackID[i]){
      //          // cout<<jentry<<"\t"<<TOF400Point_fTrackID[i]<<"\t"<<TOF400Point_fX[i]<<"\t"<<TOF400Point_fPx[i]<<endl;
      ...
      if(Nofparticles_left!=1 || Nofparticles_right!=1){
         memset(StsPoint_fPx,0,sizeof(StsPoint_fPx));
         memset(StsPoint_fPy,0,sizeof(StsPoint_fPy));
         memset(StsPoint_fPz,0,sizeof(StsPoint_fPz));
         memset(StsPoint_fX,0,sizeof(StsPoint_fX));
      ...
      // cout<<Hit_GEM_left<<"\t"<<Hit_GEM_right<<"\n"<<Hit_CSC_left<<"\t"<<Hit_CSC_right<<"\n"<<Hit_TOF_left<<"\t"<<Hit_TOF_right<<"\n"<<endl;
      ...
      if(Flag_left==1&&Flag_right==1){
         h_Pfragx_PmissTheta_both->Fill(kp.Angle(kz)/3.1415926535*180,PBX);
         h_Pfragx_OpeningTheta_both->Fill(kp1.Angle(kp2)/3.1415926535*180,PBX);
         h_Pfragy_PmissTheta_both->Fill(kp.Angle(kz)/3.1415926535*180,PBY);
         h_Pfragy_OpeningTheta_both->Fill(kp1.Angle(kp2)/3.1415926535*180,PBY);
         h_Pfragz_PmissTheta_both->Fill(kp.Angle(kz)/3.1415926535*180,PBZ);
         h_Pfragz_OpeningTheta_both->Fill(kp1.Angle(kp2)/3.1415926535*180,PBZ);
         // h_M_miss_square_frag_highstat_with_accp->Fill(M2_frag_highstat);
         // h_P_miss_frag_highstat_with_accp->Fill(Pmiss_frag_highstat);
         // h_P_miss_x_frag_highstat_with_accp->Fill(Pmiss_x_frag_highstat);
         // h_P_miss_y_frag_highstat_with_accp->Fill(Pmiss_y_frag_highstat);
         // h_P_miss_z_frag_highstat_with_accp->Fill(Pmiss_z_frag_highstat);
      ...
      if(Flag_left==1){
         // h_tof_pos_left->Fill(x_left,y_left);
         h_Pfragx_PmissTheta_left->Fill(kp.Angle(kz)/3.1415926535*180,PBX);
         h_Pfragx_OpeningTheta_left->Fill(kp1.Angle(kp2)/3.1415926535*180,PBX);
         h_Pfragy_PmissTheta_left->Fill(kp.Angle(kz)/3.1415926535*180,PBY);
         h_Pfragy_OpeningTheta_left->Fill(kp1.Angle(kp2)/3.1415926535*180,PBY);
         h_Pfragz_PmissTheta_left->Fill(kp.Angle(kz)/3.1415926535*180,PBZ);
         h_Pfragz_OpeningTheta_left->Fill(kp1.Angle(kp2)/3.1415926535*180,PBZ);
         ...
         if(Smearing_Flag==1) TOF_left = gRandom->Gaus(TOF_left,TOF_resolution/1000.);   //ns
         ...
         if(TOF_Momenta_Flag==1){
            Beta_left = beta_left;
            if(Beta_left<0.8||Beta_left>0.98) Flag_left = 0;
            // if(Beta_left<0.89||Beta_left>0.94) Flag_left = 0;
            Gamma_left = 1./sqrt(1-pow(Beta_left,2));
            P_left = p_left;
            Px_left = px_left;
            Py_left = py_left;
         ...
         h_Pfragz_theta_left->Fill(theta1,PBZ);

         P4_left_initial.SetPxPyPzE(Px_left,Py_left,Pz_left,E_left);
         P4_left_transformed = boost *P4_left_initial;
      }
      if(Flag_right==1){
         // h_tof_pos_right->Fill(x_right,y_right);
         h_Pfragx_PmissTheta_right->Fill(kp.Angle(kz)/3.1415926535*180,PBX);
         h_Pfragx_OpeningTheta_right->Fill(kp1.Angle(kp2)/3.1415926535*180,PBX);
         h_Pfragy_PmissTheta_right->Fill(kp.Angle(kz)/3.1415926535*180,PBY);
         h_Pfragy_OpeningTheta_right->Fill(kp1.Angle(kp2)/3.1415926535*180,PBY);
         h_Pfragz_PmissTheta_right->Fill(kp.Angle(kz)/3.1415926535*180,PBZ);
         h_Pfragz_OpeningTheta_right->Fill(kp1.Angle(kp2)/3.1415926535*180,PBZ);
         ...
         if(Smearing_Flag==1) TOF_right = gRandom->Gaus(TOF_right,TOF_resolution/1000.);   //ns
         ...
         if(TOF_Momenta_Flag==1){
            Beta_right = beta_right;
            if(Beta_right<0.8||Beta_right>0.98) Flag_right = 0;
            // if(Beta_right<0.92||Beta_right>0.97) Flag_right = 0;
            Gamma_right = 1./sqrt(1-pow(Beta_right,2));
            P_right = p_right;
            Px_right = px_right;
            Py_right = py_right;
         ...
         h_Pfragz_theta_right->Fill(theta2,PBZ);

         P4_right_initial.SetPxPyPzE(Px_right,Py_right,Pz_right,E_right);
         P4_right_transformed = boost *P4_right_initial;
      }
      if(Flag_left==1&&Flag_right==1){
         vec1.SetXYZ(Px_left, Py_left, Pz_left);
         vec2.SetXYZ(Px_right, Py_right, Pz_right);
         ...
         h_Pfragz_theta_left_both->Fill(theta1,PBZ);
         h_Pfragz_theta_right_both->Fill(theta2,PBZ);

         h_px_diff_polar->Fill(theta1_x,px1_diff); h_px_diff_polar->Fill(theta2_x,px2_diff);
         h_py_diff_polar->Fill(theta1_y,py1_diff); h_py_diff_polar->Fill(theta2_y,py2_diff);
         h_pz_diff_polar->Fill(theta1_1,pz1_diff); h_pz_diff_polar->Fill(theta2_1,pz2_diff);
         h_P_miss_x_diff_polar->Fill(theta1_x+theta2_x,P_miss_x_diff);
         h_P_miss_y_diff_polar->Fill(theta1_y+theta2_y,P_miss_y_diff);
         h_P_miss_z_diff_polar->Fill(theta1_1+theta2_1,P_miss_z_diff);
         ...
         h_fly_path_x_left->Fill(LOF_x_left);
         h_fly_path_y_left->Fill(LOF_y_left);
         h_fly_path_z_left->Fill(LOF_z_left);
         h_fly_path_left->Fill(LOF_left);
         h_fly_path_x_right->Fill(LOF_x_right);
         h_fly_path_y_right->Fill(LOF_y_right);
         h_fly_path_z_right->Fill(LOF_z_right);
         h_fly_path_right->Fill(LOF_right);
         h_fly_time_left->Fill(TOF_left);
         h_fly_time_right->Fill(TOF_right);
         ...
         if(fabs((theta1+theta2 - thetaMean) / thetaSigma)<2.){
            h_E_miss_with_tail->Fill(E_miss);
            // h_M_miss_square->Fill(M_miss*M_miss);
         }
         ...
         // if((M_miss*M_miss<0.7 || M_miss*M_miss>1.1) &&QE_Cut_Flag) Continue_Flag=1;

         for(int nn=Current11BNo;nn<Numof11B;nn++){
            Current11BNo = nn;
            if(EvtNo_11B[nn]>=NN) break;
         }
         if(EvtNo_11B[Current11BNo]>NN) Continue_Flag=1;
         ...
         // TVector3 rotationAxis = Pmiss.Cross(Pfrag);
         // if(rotationAxis.X()>0) Pfrag.RotateX(0.1);
         // else Pfrag.RotateX(-0.1);
         // if(rotationAxis.Y()>0) Pfrag.RotateY(0.1);
         // else Pfrag.RotateY(-0.1);
         // if(rotationAxis.Z()>0) Pfrag.RotateZ(0.1);
         // else Pfrag.RotateZ(-0.1);

         angle_pmiss_pfrag = Pmiss.Angle(Pfrag);
         // angle_pmiss_pfrag -= 0.1;
         ...
         h_Angle_Pmiss_PFrag->Fill(angle_pmiss_pfrag*180.0/TMath::Pi());
         h_Angle_Pmiss_PFrag_cos->Fill(cos(angle_pmiss_pfrag));
         h_Angle_Pmiss_PFrag_cos_2D->Fill(cos(angle_pmiss_pfrag_2D));
         h_Pmiss_Pfrag_x->Fill(P_miss_x,PB_transformed.Px());
         h_Pmiss_Pfrag_y->Fill(P_miss_y,PB_transformed.Py());
         h_Pmiss_Pfrag_z->Fill(P_miss_z,PB_transformed.Pz());
         h_Pmiss_Pfrag_tot->Fill(P_miss,sqrt(pow(PB_transformed.Px(),2)+pow(PB_transformed.Py(),2)+pow(PB_transformed.Pz(),2)));

         // h_pbx->Fill(PB_transformed.Px());
         // h_pby->Fill(PB_transformed.Py());
         // double P_survive = 1 - breakup_possibility(-576.2+15-vertex_z_1);
         // double Value_random = gRandom->Uniform(0,1);
         // if(Value_random<P_survive){
            h_pbx->Fill(PB_initial.Px());
            h_pby->Fill(PB_initial.Py());
            h_pbz->Fill(PB_transformed.Pz());
            h_pb->Fill(sqrt(pow(PB_transformed.Px(),2)+pow(PB_transformed.Py(),2)+pow(PB_transformed.Pz(),2)));
            h_vertex_z_breakup->Fill(vertex_z);
         // }
         ...
         double max_t_u = std::max(fabs(tt),fabs(uu));

         // if(max_t_u>0&&max_t_u<2.9) h_scattering_angle->Fill(CA);
         // if(max_t_u>2.9&&max_t_u<3.4) h_scattering_angle->Fill(CA);
         // if(max_t_u>3.4&&max_t_u<4) h_scattering_angle->Fill(CA);
         // if(max_t_u>4) h_scattering_angle->Fill(CA);

         // if(max_t_u>2.0&&max_t_u<2.5) h_scattering_angle->Fill(CA);
         // if(max_t_u>2.5&&max_t_u<3.0) h_scattering_angle->Fill(CA);
         // if(max_t_u>3.0&&max_t_u<3.5) h_scattering_angle->Fill(CA);
         // if(max_t_u>3.5&&max_t_u<4.0) h_scattering_angle->Fill(CA);
         // if(max_t_u>4.0&&max_t_u<4.5) h_scattering_angle->Fill(CA);
         // if(max_t_u>4.5&&max_t_u<5.0) h_scattering_angle->Fill(CA);
         // if(max_t_u>5.0&&max_t_u<6.0) h_scattering_angle->Fill(CA);

         h_scattering_angle->Fill(CA);
         h_scattering_angle_1->Fill(CA_1);
         ...
         for(int nn=CurrentdeltaNo;nn<Numofdelta;nn++){
            CurrentdeltaNo = nn;

            int NN = EvtNo_delta[nn];
            if(NN<jentry) continue;

            if(NN==jentry){
               // cout<<"!!!"<<endl;
               No_exist_flag = 1;
               TVector3 vec(delta_x[nn], delta_y[nn], delta_z[nn]);
         
               double dotProduct_p1 = vec.Dot(Vec1);
               double dotProduct_p2 = vec.Dot(Vec2);

               double magnitude = vec.Mag();
               double magnitude1 = Vec1.Mag();
               double magnitude2 = Vec2.Mag();
               ...
               // if( (angleDeg_p1<20 && amp_diff_p1<0.1) || (angleDeg_p2<20 && amp_diff_p2<0.1) ) delta_flag = 1;
               if( (angleDeg_p1<10 && amp_diff_p1<0.05) || (angleDeg_p2<10 && amp_diff_p2<0.05) ) delta_flag = 1;
               // if( (angleDeg_p1<1 && amp_diff_p1<0.05) || (angleDeg_p2<1 && amp_diff_p2<0.05) ) delta_flag = 1;
               // if(delta_flag==1) cout<<Vec1[0]<<"\t"<<Vec2[0]<<"\t"<<vec[0]<<endl;
            }

            if(NN>jentry && No_exist_flag==0){
               delta_flag = 0;
               break;
            }
            if(NN>jentry) break;
         ...
         // if(EvtNo_11B[CurrentdeltaNo]>NN) Continue_Flag=1;

         if(delta_pion_flag==1){
            NEvents_from_delta++;
            NonQE_Flag = 1;
            // cout<<pdg_test<<endl;
         }
         else if(delta_flag==1){
            NEvents_from_delta++;
            NonQE_Flag = 1;
         }
         ...
         if(CA>70.&&CA<110.) {
            // NEvents_With_cuts++;
            // if(max_t_u>0&&max_t_u<2.9) NEvents_With_cuts++;
            // if(max_t_u>2.9&&max_t_u<3.4) NEvents_With_cuts++;
            // if(max_t_u>3.4&&max_t_u<4) NEvents_With_cuts++;
            // if(max_t_u>4) NEvents_With_cuts++;

            // if(max_t_u>2.0&&max_t_u<2.5) NEvents_With_cuts++;
            // if(max_t_u>2.5&&max_t_u<3.0) NEvents_With_cuts++;
            // if(max_t_u>3.0&&max_t_u<3.5) NEvents_With_cuts++;
            // if(max_t_u>3.5&&max_t_u<4.0) NEvents_With_cuts++;
            // if(max_t_u>4.0&&max_t_u<4.5) NEvents_With_cuts++;
            // if(max_t_u>4.5&&max_t_u<5.0) NEvents_With_cuts++;
            // if(max_t_u>5.0&&max_t_u<6.0) NEvents_With_cuts++;
         }
         if(CA>85.&&CA<95.) h_vertex_z_with_cut->Fill(vertex_z);
         h_Mandelstam_t->Fill(fabs(tt));
         h_Mandelstam_u->Fill(fabs(uu));
         h_Mandelstam_u_t->Fill(std::max(fabs(tt),fabs(uu))); 
         h_Mandelstam_u_t_min->Fill(std::min(fabs(tt),fabs(uu))); 
         // h_Mandelstam_u_t_min_1->Fill(std::min(fabs(tt_1),fabs(uu_1))); 
         // h_Mandelstam_t_1->Fill(fabs(tt_1));
         // h_Mandelstam_u_1->Fill(fabs(uu_1));
         // h_Mandelstam_u_t_1->Fill(std::max(fabs(tt_1),fabs(uu_1)));
         ...
         h_p1x_diff_1->Fill(Px_left-px_left);
         h_p1y_diff_1->Fill(Py_left-py_left);
         h_p1z_diff_1->Fill(Pz_left-pz_left);
         h_p1_diff_1->Fill(P_left-p_left);   
         h_p2x_diff_1->Fill(Px_right-px_right);
         h_p2y_diff_1->Fill(Py_right-py_right);
         h_p2z_diff_1->Fill(Pz_right-pz_right);
         h_p2_diff_1->Fill(P_right-p_right);
         h_p_miss_x_diff_1->Fill(P_miss_x-P_miss_x_tof);
         h_p_miss_y_diff_1->Fill(P_miss_y-P_miss_y_tof);
         h_p_miss_z_diff_1->Fill(P_miss_z-P_miss_z_tof);
         h_p_miss_diff_1->Fill(P_miss-P_miss_tof);
         h_E_miss_diff_1->Fill(E_miss-E_miss_tof);
         h_M_miss_diff_1->Fill(M_miss-M_miss_tof);
         h_M_miss_square_diff_1->Fill(M_miss*M_miss-M_miss_tof*M_miss_tof);

         // Fill difference between reco and generator
         h_p1x_diff->Fill(Px_left-px_left_initial);
         h_p1y_diff->Fill(Py_left-py_left_initial);
         h_p1z_diff->Fill(Pz_left-pz_left_initial);
         h_p1_diff->Fill(P_left-p_left_initial);   
         h_p2x_diff->Fill(Px_right-px_right_initial);
         h_p2y_diff->Fill(Py_right-py_right_initial);
         h_p2z_diff->Fill(Pz_right-pz_right_initial);
         h_p2_diff->Fill(P_right-p_right_initial);
         h_p_miss_x_diff->Fill(P_miss_x-P_miss_x_1);
         h_p_miss_y_diff->Fill(P_miss_y-P_miss_y_1);
         h_p_miss_z_diff->Fill(P_miss_z-P_miss_z_1);
         h_p_miss_diff->Fill(P_miss-P_miss_1);
         h_p_miss_diff_over_pmiss->Fill((P_miss-P_miss_1)/P_miss);
         h_E_miss_diff->Fill(E_miss-E_miss_1);
         h_M_miss_diff->Fill(M_miss-M_miss_1);
         h_M_miss_square_diff->Fill(M_miss*M_miss-M_miss_1*M_miss_1);
         h_Trieman_Yang_Angle->Fill(cosTheta);
         ...
         h_p2z->Fill(Pz_right);
         h_E2->Fill(P_right);
         h_p1_minus_p2->Fill(P_left-P_right);
         h_p1x_minus_p2x->Fill(Px_left-Px_right);
         h_p1y_minus_p2y->Fill(Py_left-Py_right);
         h_p1z_minus_p2z->Fill(Pz_left-Pz_right);
         h_p1_and_p2->Fill(P_left+P_right);
         h_p1x_and_p2x->Fill(Px_left+Px_right);
         h_p1y_and_p2y->Fill(Py_left+Py_right);
         h_p1z_and_p2z->Fill(Pz_left+Pz_right);
         ...
         h_M_miss_square->Fill(M_miss*M_miss);
         h_opening_theta->Fill(theta1+theta2);
         h_opening_theta_Emiss->Fill(E_miss,theta1+theta2);
         h_opening_theta_Mmiss2->Fill(M_miss*M_miss,theta1+theta2);
         opening_theta = theta1 + theta2;
         if(phi1-phi2>-180 && phi1-phi2<180){
            h_opening_phi->Fill(phi1-phi2);
            h_opening_phi_Emiss->Fill(E_miss,phi1-phi2);  
         }
         else if(phi1-phi2<-180){
            h_opening_phi->Fill(phi1-phi2+360);
            h_opening_phi_Emiss->Fill(E_miss,phi1-phi2+360);
         }
         else{
            h_opening_phi->Fill(phi1-phi2-360);
            h_opening_phi_Emiss->Fill(E_miss,phi1-phi2-360);
         ...
         h_phi1->Fill(phi1);
         h_phi2->Fill(phi2);
         h_theta1->Fill(theta1);
         h_theta2->Fill(theta2);
         h_theta12->Fill(theta1,theta2);
         h_vertex_x->Fill(vertex_x);
         h_vertex_y->Fill(vertex_y);
         h_vertex_z->Fill(vertex_z);

         h_beta_left->Fill(Beta_left);
         h_beta_right->Fill(Beta_right);
         h_beta_left_vs_right->Fill(Beta_left,Beta_right);
         ...
         h_M_miss_square_frag_highstat_with_accp_diff->Fill(M2_frag_highstat-M_miss);
         h_P_miss_frag_highstat_with_accp_diff->Fill(Pmiss_frag_highstat-P_miss);
         h_P_miss_x_frag_highstat_with_accp_diff->Fill(Pmiss_x_frag_highstat-P_miss_x);
         h_P_miss_y_frag_highstat_with_accp_diff->Fill(Pmiss_y_frag_highstat-P_miss_y);
         h_P_miss_z_frag_highstat_with_accp_diff->Fill(Pmiss_z_frag_highstat-P_miss_z);

         h_Pmiss_diff_ratio->Fill((Pmiss_frag_highstat-P_miss)/Pmiss_frag_highstat*100.);
         h_Pmiss_diff_ratio_frag_highstat->Fill(Pmiss_frag_highstat,(Pmiss_frag_highstat-P_miss)/Pmiss_frag_highstat*100.);
         h_Pmiss_diff_ratio_arm->Fill(P_miss,(Pmiss_frag_highstat-P_miss)/Pmiss_frag_highstat*100.);
         h_Pmiss_frag_highstat_theta->Fill(angle_deg,Pmiss_frag_highstat);
         h_Pmiss_arm_theta->Fill(angle_deg,P_miss);

         h_M_miss_square_frag_highstat_with_accp_vs_method1->Fill(M2_frag_highstat,M_miss);
         // h_P_miss_frag_highstat_with_accp_vs_method1->Fill(Pmiss_frag_highstat,P_miss);
         // h_P_miss_x_frag_highstat_with_accp_vs_method1->Fill(Pmiss_x_frag_highstat,P_miss_x);
         // h_P_miss_y_frag_highstat_with_accp_vs_method1->Fill(Pmiss_y_frag_highstat,P_miss_y);
         // h_P_miss_z_frag_highstat_with_accp_vs_method1->Fill(Pmiss_z_frag_highstat,P_miss_z);
         h_P_miss_frag_highstat_with_accp_vs_method1->Fill(P_miss,Pmiss_frag_highstat);
         h_P_miss_x_frag_highstat_with_accp_vs_method1->Fill(P_miss_x,Pmiss_x_frag_highstat);
         h_P_miss_y_frag_highstat_with_accp_vs_method1->Fill(P_miss_y,Pmiss_y_frag_highstat);
         h_P_miss_z_frag_highstat_with_accp_vs_method1->Fill(P_miss_z,Pmiss_z_frag_highstat);
         //--Comparation between reconstruction form fragments and reconstruction form protons
         ...
         // h_phi1_1->Fill(phi1_1);
         // h_phi2_1->Fill(phi2_1);
         // h_vertex_z_1->Fill(vertex_z_1);

         if(CA_1>60.&&CA_1<120.){
            h_theta12_accp->Fill(theta1_1,theta2_1);
            h_phi12_accp->Fill(phi1_1,phi2_1);
            h_theta_phi_accp->Fill(theta1_1,phi1_1);
            h_theta_phi1_accp->Fill(theta1_1,phi1_1);
            h_theta_phi_accp->Fill(theta2_1,phi2_1);
            h_theta_phi2_accp->Fill(theta2_1,phi2_1);
         }
      }
      //---------------------------Calculate momentum using TOF-------------------------
   ...
   // TFile *file_output_QE = new TFile("./QE_Simulation.root","RECREATE"); // "UPDATE"
   file_output_QE->cd();
   tree_QE->Write();
   // tree_Generator->Write();
   h_opening_theta_Emiss->Write();
   h_opening_theta_Mmiss2->Write();
   h_opening_phi_Emiss->Write();
   h_Trieman_Yang_Angle->Write();
   file_output_QE->Close();
   delete file_output_QE;

   TFile *My_output = new TFile("./my_output_delta_test_frag_highstat.root", "RECREATE");
   h_vertex_z->Write();
   h_vertex_z_breakup->Write();
   h_p1x->Write();
   h_p1y->Write();
   h_p1z->Write();
   h_E1->Write();
   ...
   TFile *My_output_generator = new TFile("./my_output_generator_delta_test_frag_highstat.root", "RECREATE");
   h_p1x_1->Write();
   h_p1y_1->Write();
   h_p1z_1->Write();
   h_E1_1->Write();
   ...
   TCanvas *c_p_miss_diff_over_pmiss = new TCanvas("c_p_miss_diff_over_pmiss","p_miss_diff_over_pmiss",1200,800);
   h_p_miss_diff_over_pmiss->Draw();
   c_p_miss_diff_over_pmiss->SaveAs(Form("./reco_plot_%3.1f_%2.0f/12_P_miss_diff_over_Pmiss.pdf",Boost_resolution,TOF_resolution));
   ...
   // ---------------PLOT reconstruction momentum of protons----------------
   TCanvas *c_p1_minus_p2 = new TCanvas("c_h_p1_minus_p2","h_p1_minus_p2",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   h_p1_minus_p2->SetLineColor(1);
   h_p1_minus_p2->Draw();
   h_p1_minus_p2->GetYaxis()->SetRangeUser(0,50*NormFactor/1243.);
   TH1F *h_p1_minus_p2_3 = (TH1F*)gROOT->FindObject("hPLMinusPR_Lab"); // hPLXMinusPRX_Lab
   if(Experiment_Flag==1){
      h_p1_minus_p2_3->SetLineColor(6);
      h_p1_minus_p2_3->Draw("Same");
      h_p1_minus_p2_3->SetNormFactor(NormFactor);
   }
	TLegend *l_p1_minus_p2 = new TLegend(0.11,0.7,0.35,0.89,Form(""));
	l_p1_minus_p2->SetFillColor(0);
	l_p1_minus_p2->SetBorderSize(0);
	l_p1_minus_p2->SetTextSize(0.03);
	l_p1_minus_p2->SetTextFont(32);
   l_p1_minus_p2->AddEntry(h_p1_minus_p2,"Simulation","l");
   if(Experiment_Flag==1){
      l_p1_minus_p2->AddEntry(h_p1_minus_p2_3,"Experiment July 30","l");
   }
   l_p1_minus_p2->Draw();
   c_p1_minus_p2->SaveAs(Form("./reco_plot_%3.1f_%2.0f/1_p1_minus_p2.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_p1x_minus_p2x = new TCanvas("c_h_p1x_minus_p2x","h_p1x_minus_p2x",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   h_p1x_minus_p2x->SetLineColor(1);
   h_p1x_minus_p2x->Draw();
   h_p1x_minus_p2x->GetYaxis()->SetRangeUser(0,50*NormFactor/1243.);
   TH1F *h_p1x_minus_p2x_3 = (TH1F*)gROOT->FindObject("hPLXMinusPRX_Lab");
   if(Experiment_Flag==1){
      h_p1x_minus_p2x_3->SetLineColor(6);
      h_p1x_minus_p2x_3->Draw("Same");
      h_p1x_minus_p2x_3->SetNormFactor(NormFactor);
   }
	TLegend *l_p1x_minus_p2x = new TLegend(0.11,0.7,0.35,0.89,Form(""));
	l_p1x_minus_p2x->SetFillColor(0);
	l_p1x_minus_p2x->SetBorderSize(0);
	l_p1x_minus_p2x->SetTextSize(0.03);
	l_p1x_minus_p2x->SetTextFont(32);
   l_p1x_minus_p2x->AddEntry(h_p1x_minus_p2x,"Simulation","l");
   if(Experiment_Flag==1){
      l_p1x_minus_p2x->AddEntry(h_p1x_minus_p2x_3,"Experiment July 30","l");
   }
   l_p1x_minus_p2x->Draw();
   c_p1x_minus_p2x->SaveAs(Form("./reco_plot_%3.1f_%2.0f/1_p1x_minus_p2x.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_p1y_minus_p2y = new TCanvas("c_h_p1y_minus_p2y","h_p1y_minus_p2y",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   h_p1y_minus_p2y->SetLineColor(1);
   h_p1y_minus_p2y->Draw();
   h_p1y_minus_p2y->GetYaxis()->SetRangeUser(0,40*NormFactor/1243.);
   TH1F *h_p1y_minus_p2y_3 = (TH1F*)gROOT->FindObject("hPLYMinusPRY_Lab");
   if(Experiment_Flag==1){
      h_p1y_minus_p2y_3->SetLineColor(6);
      h_p1y_minus_p2y_3->Draw("Same");
      h_p1y_minus_p2y_3->SetNormFactor(NormFactor);
   }
	TLegend *l_p1y_minus_p2y = new TLegend(0.11,0.7,0.35,0.89,Form(""));
	l_p1y_minus_p2y->SetFillColor(0);
	l_p1y_minus_p2y->SetBorderSize(0);
	l_p1y_minus_p2y->SetTextSize(0.03);
	l_p1y_minus_p2y->SetTextFont(32);
   l_p1y_minus_p2y->AddEntry(h_p1y_minus_p2y,"Simulation","l");
   if(Experiment_Flag==1){
      l_p1y_minus_p2y->AddEntry(h_p1y_minus_p2y_3,"Experiment July 30","l");
   }
   l_p1y_minus_p2y->Draw();
   c_p1y_minus_p2y->SaveAs(Form("./reco_plot_%3.1f_%2.0f/1_p1y_minus_p2y.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_p1z_minus_p2z = new TCanvas("c_h_p1z_minus_p2z","h_p1z_minus_p2z",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   h_p1z_minus_p2z->SetLineColor(1);
   h_p1z_minus_p2z->Draw();
   h_p1z_minus_p2z->GetYaxis()->SetRangeUser(0,40*NormFactor/1243.);
   TH1F *h_p1z_minus_p2z_3 = (TH1F*)gROOT->FindObject("hPLZMinusPRZ_Lab");
   if(Experiment_Flag==1){
      h_p1z_minus_p2z_3->SetLineColor(6);
      h_p1z_minus_p2z_3->Draw("Same");
      h_p1z_minus_p2z_3->SetNormFactor(NormFactor);
   }
	TLegend *l_p1z_minus_p2z = new TLegend(0.11,0.7,0.35,0.89,Form(""));
	l_p1z_minus_p2z->SetFillColor(0);
	l_p1z_minus_p2z->SetBorderSize(0);
	l_p1z_minus_p2z->SetTextSize(0.03);
	l_p1z_minus_p2z->SetTextFont(32);
   l_p1z_minus_p2z->AddEntry(h_p1z_minus_p2z,"Simulation","l");
   if(Experiment_Flag==1){
      l_p1z_minus_p2z->AddEntry(h_p1z_minus_p2z_3,"Experiment July 30","l");
   }
   l_p1z_minus_p2z->Draw();
   c_p1z_minus_p2z->SaveAs(Form("./reco_plot_%3.1f_%2.0f/1_p1z_minus_p2z.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_p1x_and_p2x = new TCanvas("c_h_p1x_and_p2x","h_p1x_and_p2x",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   h_p1x_and_p2x->SetLineColor(1);
   h_p1x_and_p2x->Draw();
   h_p1x_and_p2x->GetYaxis()->SetRangeUser(0,70*NormFactor/1243.);
   TH1F *h_p1x_and_p2x_3 = (TH1F*)gROOT->FindObject("hPLXPlusPRX_Lab");
   if(Experiment_Flag==1){
      h_p1x_and_p2x_3->SetLineColor(6);
      h_p1x_and_p2x_3->Draw("Same");
      h_p1x_and_p2x_3->SetNormFactor(NormFactor);
   }
	TLegend *l_p1x_and_p2x = new TLegend(0.11,0.7,0.35,0.89,Form(""));
	l_p1x_and_p2x->SetFillColor(0);
	l_p1x_and_p2x->SetBorderSize(0);
	l_p1x_and_p2x->SetTextSize(0.03);
	l_p1x_and_p2x->SetTextFont(32);
   l_p1x_and_p2x->AddEntry(h_p1x_and_p2x,"Simulation","l");
   if(Experiment_Flag==1){
      l_p1x_and_p2x->AddEntry(h_p1x_and_p2x_3,"Experiment July 30","l");
   }
   l_p1x_and_p2x->Draw();
   c_p1x_and_p2x->SaveAs(Form("./reco_plot_%3.1f_%2.0f/1_p1x_plus_p2x.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_p1y_and_p2y = new TCanvas("c_h_p1y_and_p2y","h_p1y_and_p2y",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   h_p1y_and_p2y->SetLineColor(1);
   h_p1y_and_p2y->Draw();
   h_p1y_and_p2y->GetYaxis()->SetRangeUser(0,90*NormFactor/1243.);
   TH1F *h_p1y_and_p2y_3 = (TH1F*)gROOT->FindObject("hPLYPlusPRY_Lab");
   if(Experiment_Flag==1){
      h_p1y_and_p2y_3->SetLineColor(6);
      h_p1y_and_p2y_3->Draw("Same");
      h_p1y_and_p2y_3->SetNormFactor(NormFactor);
   }
	TLegend *l_p1y_and_p2y = new TLegend(0.11,0.7,0.35,0.89,Form(""));
	l_p1y_and_p2y->SetFillColor(0);
	l_p1y_and_p2y->SetBorderSize(0);
	l_p1y_and_p2y->SetTextSize(0.03);
	l_p1y_and_p2y->SetTextFont(32);
   l_p1y_and_p2y->AddEntry(h_p1y_and_p2y,"Simulation","l");
   if(Experiment_Flag==1){
      l_p1y_and_p2y->AddEntry(h_p1y_and_p2y_3,"Experiment July 30","l");
   }
   l_p1y_and_p2y->Draw();
   c_p1y_and_p2y->SaveAs(Form("./reco_plot_%3.1f_%2.0f/1_p1y_plus_p2y.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_p1z_and_p2z = new TCanvas("c_h_p1z_and_p2z","h_p1z_and_p2z",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   h_p1z_and_p2z->SetLineColor(1);
   h_p1z_and_p2z->Draw();
   h_p1z_and_p2z->GetYaxis()->SetRangeUser(0,40*NormFactor/1243.);
   TH1F *h_p1z_and_p2z_3 = (TH1F*)gROOT->FindObject("hPLZPlusPRZ_Lab");
   if(Experiment_Flag==1){
      h_p1z_and_p2z_3->SetLineColor(6);
      h_p1z_and_p2z_3->Draw("Same");
      h_p1z_and_p2z_3->SetNormFactor(NormFactor);
   }
	TLegend *l_p1z_and_p2z = new TLegend(0.11,0.7,0.35,0.89,Form(""));
	l_p1z_and_p2z->SetFillColor(0);
	l_p1z_and_p2z->SetBorderSize(0);
	l_p1z_and_p2z->SetTextSize(0.03);
	l_p1z_and_p2z->SetTextFont(32);
   l_p1z_and_p2z->AddEntry(h_p1z_and_p2z,"Simulation","l");
   if(Experiment_Flag==1){
      l_p1z_and_p2z->AddEntry(h_p1z_and_p2z_3,"Experiment July 30","l");
   }
   l_p1z_and_p2z->Draw();
   c_p1z_and_p2z->SaveAs(Form("./reco_plot_%3.1f_%2.0f/1_p1z_plus_p2z.pdf",Boost_resolution,TOF_resolution));
   ...
   if(Experiment_Flag==1){
      l_p1x->AddEntry(h_p1x_3,"Experiment July 30","l");
   }
   l_p1x->Draw();
   ...
   if(Experiment_Flag==1){
      l_p1y->AddEntry(h_p1y_3,"Experiment July 30","l");
   }
   l_p1y->Draw();
   ...
   if(Experiment_Flag==1){
      l_p1z->AddEntry(h_p1z_3,"Experiment July 30","l");
   }
   l_p1z->Draw();
   ...
   if(Experiment_Flag==1){
      l_E1->AddEntry(h_E1_3,"Experiment July 30","l");
   }
   l_E1->Draw();
   ...
   if(Experiment_Flag==1){
      l_p2x->AddEntry(h_p2x_3,"Experiment July 30","l");
   }
   l_p2x->Draw();
   ...
   if(Experiment_Flag==1){
      l_p2y->AddEntry(h_p2y_3,"Experiment July 30","l");
   }
   l_p2y->Draw();
   ...
   if(Experiment_Flag==1){
      l_p2z->AddEntry(h_p2z_3,"Experiment July 30","l");
   }
   l_p2z->Draw();
   ...
   if(Experiment_Flag==1){
      l_E2->AddEntry(h_E2_3,"Experiment July 30","l");
   }
   l_E2->Draw();
   ...
   // l_p1xp2x->AddEntry(h_p1xp2x_3,"Experiment July 30","l");
   // l_p1xp2x->Draw();
   ...
   // l_p1yp2y->AddEntry(h_p1yp2y_3,"Experiment July 30","l");
   // l_p1yp2y->Draw();
   ...
   if(Experiment_Flag==1){
      l_p1zp2z->AddEntry(h_p1zp2z_3,"Experiment July 30","l");
   }
   l_p1zp2z->Draw();
   ...
   // l_P_miss->AddEntry(h_P_miss_3,"Experiment July 30","l");
   // l_P_miss->Draw();
   ...
   if(Experiment_Flag==1){
      h_P_miss_3->SetLineColor(6);
      h_P_miss_3->Draw("Same");
      h_P_miss_3->SetNormFactor(NormFactor);
   }
	TLegend *l_P_miss = new TLegend(0.55,0.2,0.9,0.4,Form(""));
	l_P_miss->SetFillColor(0);
	l_P_miss->SetBorderSize(1);
	l_P_miss->SetTextSize(0.03);
	l_P_miss->SetTextFont(32);
   ...
   if(Experiment_Flag==1){
      l_E_miss->AddEntry(h_E_miss_3,"Experiment July 30","l");
   }
   l_E_miss->Draw();
   ...
   if(Experiment_Flag==1){
      ...
      h_M_miss_square_3->Draw("Same");
      h_M_miss_square_3->SetNormFactor(NormFactor);
      // h_M_miss_square_3->Fit("gaus","","SAME",0.5,0.9);
   }
	TLegend *l_M_miss_square = new TLegend(0.55,0.2,0.9,0.4,Form(""));
   ...
   if(Experiment_Flag==1){
      l_M_miss_square->AddEntry(h_M_miss_square_3,"Experiment July 30","l");
   }
   l_M_miss_square->Draw();
   ...
   // double mean_M_miss = fitFunc_M_miss->GetParameter(1);
   // double sigma_M_miss = fitFunc_M_miss->GetParameter(2);
   double mean_M_miss = 1.;
   double sigma_M_miss = 1.;
   ...
   // l_opening_theta->AddEntry(h_opening_theta_3,"Experiment July 30","l");
   // l_opening_theta->Draw();
   ...
   // l_opening_phi->AddEntry(h_opening_phi_3,"Experiment July 30","l");
   // l_opening_phi->Draw();
   ...
   l_phi1->AddEntry(h_phi1_3,"Experiment July 30","l");
   l_phi1->Draw();
   ...
   l_phi2->AddEntry(h_phi2_3,"Experiment July 30","l");
   l_phi2->Draw();
   ...
   // }
   if(Experiment_Flag==1){
      l_Mandelstam_t->AddEntry(h_Mandelstam_t_3,"Experiment July 30","l");
   }
   l_Mandelstam_t->Draw();
   ...
   if(Experiment_Flag==1){
      l_Mandelstam_u->AddEntry(h_Mandelstam_u_3,"Experiment July 30","l");
   }
   // if(Generator_With_Cut==1){
   ...
   if(Experiment_Flag==1){
      l_Mandelstam_u_t->AddEntry(h_Mandelstam_u_t_3,"Experiment July 30","l");
   }
   // if(Generator_With_Cut==1){
   //    l_Mandelstam_u_t->AddEntry(h_Mandelstam_u_t_1,"Generator with acceptance cut","l");
   // }
   l_Mandelstam_u_t->Draw();
   c_Mandelstam_u_t->SaveAs(Form("./reco_plot_%3.1f_%2.0f/18_Mandelstam_max_ut.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_Mandelstam_u_t_min = new TCanvas("c_Mandelstam_u_t_min","Mandelstam_u_t_min",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   h_Mandelstam_u_t_min->SetLineColor(1);
   h_Mandelstam_u_t_min->Draw();
   h_Mandelstam_u_t_min->GetYaxis()->SetRangeUser(0,30*NormFactor/260.);
   if(Generator_With_Cut==1){
      h_Mandelstam_u_t_min_1->SetLineColor(4);
      h_Mandelstam_u_t_min_1->SetLineStyle(7);
      h_Mandelstam_u_t_min_1->Draw("Same");
   }
   TH1F *h_Mandelstam_u_t_min_3 = (TH1F*)gROOT->FindObject("hMandelstamMinTU_QE_MM");
   if(Experiment_Flag==1){
      h_Mandelstam_u_t_min_3->SetLineColor(6);
      h_Mandelstam_u_t_min_3->Draw("Same");
      h_Mandelstam_u_t_min_3->SetNormFactor(NormFactor);
   }
   TLegend *l_Mandelstam_u_t_min = new TLegend(0.15,0.7,0.65,0.85,Form(""));
	l_Mandelstam_u_t_min->SetFillColor(0);
	l_Mandelstam_u_t_min->SetBorderSize(0);
	l_Mandelstam_u_t_min->SetTextSize(0.03);
	l_Mandelstam_u_t_min->SetTextFont(32);
   l_Mandelstam_u_t_min->AddEntry(h_Mandelstam_u_t_min,"Simulation","l");
   if(Generator_With_Cut==1){
      l_Mandelstam_u_t_min->AddEntry(h_Mandelstam_u_t_min_1,"Simulation w/o smearing","l");
   }
   if(Experiment_Flag==1){
      l_Mandelstam_u_t_min->AddEntry(h_Mandelstam_u_t_min_3,"Experiment July 30","l");
   }
   l_Mandelstam_u_t_min->Draw();
   c_Mandelstam_u_t_min->SaveAs(Form("./reco_plot_%3.1f_%2.0f/18_Mandelstam_min_ut.pdf",Boost_resolution,TOF_resolution));
   ...
   gStyle->SetOptStat(1111);
   h_vertex_z->Draw();
   h_vertex_z->GetYaxis()->SetRangeUser(0,30*NormFactor/300.);
   if(Generator_With_Cut==1){
      h_vertex_z_1->SetLineColor(4);
      h_vertex_z_1->SetLineStyle(7);
      h_vertex_z_1->Draw("Same");
   }
   TH1F *h_vertex_z_3 = (TH1F*)gROOT->FindObject("hVertexZ_11B_QE");
   if(Experiment_Flag==1){
      h_vertex_z_3->SetLineColor(6);
      h_vertex_z_3->Draw("Same");
      h_vertex_z_3->SetNormFactor(NormFactor);
   }
   TLegend *l_vertex_z = new TLegend(0.15,0.7,0.45,0.85,Form(""));
	l_vertex_z->SetFillColor(0);
	l_vertex_z->SetBorderSize(0);
	l_vertex_z->SetTextSize(0.03);
	l_vertex_z->SetTextFont(32);
   l_vertex_z->AddEntry(h_vertex_z,"Simulation","l");
   if(Generator_With_Cut==1){
      l_vertex_z->AddEntry(h_vertex_z_1,"Simulation w/o smearing","l");
   }
   if(Experiment_Flag==1){
      l_vertex_z->AddEntry(h_vertex_z_3,"Experiment July 30","l");
   }
   l_vertex_z->Draw();
   ...
   if(Experiment_Flag==1){
      ...
      h_Angle_Pmiss_PFrag_cos_3->Draw("Same");
   ...
   if(Experiment_Flag==1){
      l_Angle_Pmiss_PFrag_cos->AddEntry(h_Angle_Pmiss_PFrag_cos_3,"Experiment July 30","l");
   }
   l_Angle_Pmiss_PFrag_cos->Draw();
   ...
   l_pbz->AddEntry(h_pbz,"Generator with acceptance cut","l");
   if(Generator_Without_Cut==1){
   ...
   TCanvas *c_M_miss_square_frag_highstat = new TCanvas("c_M_miss_square_frag_highstat","M_miss_square_frag_highstat",1200,800);
   gStyle->SetOptFit(11);
   gStyle->SetOptStat(11);
   h_M_miss_square_frag_highstat_with_accp->SetLineColor(4);
   h_M_miss_square_frag_highstat_with_accp->Draw();
   // h_M_miss_square_frag_highstat_with_accp->SetNormFactor(8478);
   // h_M_miss_square_frag_highstat_with_accp->Fit("gaus","","SAME",0.6,0.85);
   h_M_miss_square->SetLineColor(1);
   h_M_miss_square->Draw("Same");
   // h_M_miss_square->SetNormFactor(8478);
   // h_M_miss_square_frag_highstat->Fit("gaus","","SAME",0.5,0.9);
   // h_M_miss_square_frag_highstat->GetYaxis()->SetRangeUser(0,190*NormFactor/1243.);
   if(Experiment_Flag==1){
      h_M_miss_square_exp->SetLineColor(6);
      h_M_miss_square_exp->Draw("Same");
      // h_M_miss_square_exp->SetNormFactor(8478);
      h_M_miss_square_exp->SetNormFactor(6836);
      // h_M_miss_square_exp->Fit("gaus","","SAME",0.5,1.1);
   }
	TLegend *l_M_miss_square_frag_highstat = new TLegend(0.65,0.2,0.9,0.4,Form(""));
	l_M_miss_square_frag_highstat->SetFillColor(0);
	l_M_miss_square_frag_highstat->SetBorderSize(1);
	l_M_miss_square_frag_highstat->SetTextSize(0.03);
	l_M_miss_square_frag_highstat->SetTextFont(32);
   // l_M_miss_square_frag_highstat->AddEntry(h_M_miss_square,"Fragments - all","l");
   l_M_miss_square_frag_highstat->AddEntry(h_M_miss_square,"Protons","l");
   l_M_miss_square_frag_highstat->AddEntry(h_M_miss_square_frag_highstat_with_accp,"Fragments - accp","l");
   if(Experiment_Flag==1){
      l_M_miss_square_frag_highstat->AddEntry(h_M_miss_square_exp,"Experiment July 30","l");
   }
   l_M_miss_square_frag_highstat->Draw();
   c_M_miss_square_frag_highstat->SaveAs(Form("./reco_plot_%3.1f_%2.0f/38_M_miss_square_frag_highstat.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_P_miss_frag_highstat = new TCanvas("c_P_miss_frag_highstat","P_miss_frag_highstat",1200,800);
   gStyle->SetOptFit(11);
   gStyle->SetOptStat(11);
   h_P_miss_frag_highstat_with_accp->SetLineColor(4);
   h_P_miss_frag_highstat_with_accp->Draw();
   // h_P_miss_frag_highstat_with_accp->SetNormFactor(8478);
   // h_P_miss_frag_highstat_with_accp->Fit("gaus","","SAME",0.6,0.85);
   h_P_miss_frag_highstat_with_accp->GetYaxis()->SetRangeUser(0,NormFactor/10.*0.7);
   h_P_miss->SetLineColor(1);
   h_P_miss->Draw("Same");
   // h_P_miss_frag_highstat->SetNormFactor(8478);
   // h_P_miss_frag_highstat->Fit("gaus","","SAME",0.5,0.9);
   if(Experiment_Flag==1){
      h_P_miss_exp->SetLineColor(6);
      h_P_miss_exp->Draw("Same");
      // h_P_miss_exp->SetNormFactor(8478);
      h_P_miss_exp->SetNormFactor(6836);
      // h_P_miss_exp->Fit("gaus","","SAME",0.5,1.1);
   }
	TLegend *l_P_miss_frag_highstat = new TLegend(0.65,0.2,0.9,0.4,Form(""));
	l_P_miss_frag_highstat->SetFillColor(0);
	l_P_miss_frag_highstat->SetBorderSize(1);
	l_P_miss_frag_highstat->SetTextSize(0.03);
	l_P_miss_frag_highstat->SetTextFont(32);
   l_P_miss_frag_highstat->AddEntry(h_P_miss,"Protons","l");
   l_P_miss_frag_highstat->AddEntry(h_P_miss_frag_highstat_with_accp,"Fragments - accp","l");
   if(Experiment_Flag==1){
      l_P_miss_frag_highstat->AddEntry(h_P_miss_exp,"Experiment July 30","l");
   }
   l_P_miss_frag_highstat->Draw();
   c_P_miss_frag_highstat->SaveAs(Form("./reco_plot_%3.1f_%2.0f/38_P_miss_frag_highstat.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_P_miss_x_frag_highstat = new TCanvas("c_P_miss_x_frag_highstat","P_miss_x_frag_highstat",1200,800);
   gStyle->SetOptFit(11);
   gStyle->SetOptStat(11);
   h_P_miss_x_frag_highstat_with_accp->SetLineColor(4);
   h_P_miss_x_frag_highstat_with_accp->Draw();
   // h_P_miss_x_frag_highstat_with_accp->SetNormFactor(8478);
   // h_P_miss_x_frag_highstat_with_accp->Fit("gaus","","SAME",0.6,0.85);
   h_p1xp2x->SetLineColor(1);
   h_p1xp2x->Draw("Same");
   // h_P_miss_x_frag_highstat->SetNormFactor(8478);
   // h_P_miss_x_frag_highstat->Fit("gaus","","SAME",0.5,0.9);
   // h_P_miss_x_frag_highstat->GetYaxis()->SetRangeUser(0,190*NormFactor/1243.);
   if(Experiment_Flag==1){
      h_P_miss_x_exp->SetLineColor(6);
      h_P_miss_x_exp->Draw("Same");
      h_P_miss_x_exp->SetNormFactor(6836);
      // h_P_miss_x_exp->Fit("gaus","","SAME",0.5,1.1);
   }
	TLegend *l_P_miss_x_frag_highstat = new TLegend(0.65,0.2,0.9,0.4,Form(""));
	l_P_miss_x_frag_highstat->SetFillColor(0);
	l_P_miss_x_frag_highstat->SetBorderSize(1);
	l_P_miss_x_frag_highstat->SetTextSize(0.03);
	l_P_miss_x_frag_highstat->SetTextFont(32);
   l_P_miss_x_frag_highstat->AddEntry(h_p1xp2x,"Protons","l");
   l_P_miss_x_frag_highstat->AddEntry(h_P_miss_x_frag_highstat_with_accp,"Fragments - accp","l");
   if(Experiment_Flag==1){
      l_P_miss_x_frag_highstat->AddEntry(h_P_miss_x_exp,"Experiment July 30","l");
   }
   l_P_miss_x_frag_highstat->Draw();
   c_P_miss_x_frag_highstat->SaveAs(Form("./reco_plot_%3.1f_%2.0f/38_P_miss_x_frag_highstat.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_P_miss_y_frag_highstat = new TCanvas("c_P_miss_y_frag_highstat","P_miss_y_frag_highstat",1200,800);
   gStyle->SetOptFit(11);
   gStyle->SetOptStat(11);
   h_P_miss_y_frag_highstat_with_accp->SetLineColor(4);
   h_P_miss_y_frag_highstat_with_accp->Draw();
   // h_P_miss_y_frag_highstat_with_accp->SetNormFactor(8478);
   // h_P_miss_y_frag_highstat_with_accp->Fit("gaus","","SAME",0.6,0.85);
   h_p1yp2y->SetLineColor(1);
   h_p1yp2y->Draw("Same");
   // h_P_miss_y_frag_highstat->SetNormFactor(8478);
   // h_P_miss_y_frag_highstat->Fit("gaus","","SAME",0.5,0.9);
   // h_P_miss_y_frag_highstat->GetYaxis()->SetRangeUser(0,190*NormFactor/1243.);
   if(Experiment_Flag==1){
      h_P_miss_y_exp->SetLineColor(6);
      h_P_miss_y_exp->Draw("Same");
      h_P_miss_y_exp->SetNormFactor(6836);
      // h_P_miss_y_exp->Fit("gaus","","SAME",0.5,1.1);
   }
	TLegend *l_P_miss_y_frag_highstat = new TLegend(0.65,0.2,0.9,0.4,Form(""));
	l_P_miss_y_frag_highstat->SetFillColor(0);
	l_P_miss_y_frag_highstat->SetBorderSize(1);
	l_P_miss_y_frag_highstat->SetTextSize(0.03);
	l_P_miss_y_frag_highstat->SetTextFont(32);
   l_P_miss_y_frag_highstat->AddEntry(h_p1yp2y,"Protons","l");
   l_P_miss_y_frag_highstat->AddEntry(h_P_miss_y_frag_highstat_with_accp,"Fragments - accp","l");
   if(Experiment_Flag==1){
      l_P_miss_y_frag_highstat->AddEntry(h_P_miss_y_exp,"Experiment July 30","l");
   }
   l_P_miss_y_frag_highstat->Draw();
   c_P_miss_y_frag_highstat->SaveAs(Form("./reco_plot_%3.1f_%2.0f/38_P_miss_y_frag_highstat.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_P_miss_z_frag_highstat = new TCanvas("c_P_miss_z_frag_highstat","P_miss_z_frag_highstat",1200,800);
   gStyle->SetOptFit(11);
   gStyle->SetOptStat(11);
   h_P_miss_z_frag_highstat_with_accp->SetLineColor(4);
   h_P_miss_z_frag_highstat_with_accp->Draw();
   // h_P_miss_z_frag_highstat_with_accp->SetNormFactor(8478);
   // h_P_miss_z_frag_highstat_with_accp->Fit("gaus","","SAME",0.6,0.85);
   h_P_miss_z_frag_highstat_with_accp->GetYaxis()->SetRangeUser(0,NormFactor/10.*1.2);
   h_p1zp2z->SetLineColor(1);
   h_p1zp2z->Draw("Same");
   // h_P_miss_z_frag_highstat->SetNormFactor(8478);
   // h_P_miss_z_frag_highstat->Fit("gaus","","SAME",0.5,0.9);
   // h_P_miss_z_frag_highstat->GetYaxis()->SetRangeUser(0,190*NormFactor/1243.);
   if(Experiment_Flag==1){
      h_P_miss_z_exp->SetLineColor(6);
      h_P_miss_z_exp->Draw("Same");
      h_P_miss_z_exp->SetNormFactor(6836);
      // h_P_miss_z_exp->Fit("gaus","","SAME",0.5,1.1);
   }
	TLegend *l_P_miss_z_frag_highstat = new TLegend(0.65,0.2,0.9,0.4,Form(""));
	l_P_miss_z_frag_highstat->SetFillColor(0);
	l_P_miss_z_frag_highstat->SetBorderSize(1);
	l_P_miss_z_frag_highstat->SetTextSize(0.03);
	l_P_miss_z_frag_highstat->SetTextFont(32);
   l_P_miss_z_frag_highstat->AddEntry(h_p1zp2z,"Protons","l");
   l_P_miss_z_frag_highstat->AddEntry(h_P_miss_z_frag_highstat_with_accp,"Fragments - accp","l");
   if(Experiment_Flag==1){
      l_P_miss_z_frag_highstat->AddEntry(h_P_miss_z_exp,"Experiment July 30","l");
   }
   l_P_miss_z_frag_highstat->Draw();
   c_P_miss_z_frag_highstat->SaveAs(Form("./reco_plot_%3.1f_%2.0f/38_P_miss_z_frag_highstat.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_M_miss_square_frag_highstat_diff = new TCanvas("c_M_miss_square_frag_highstat_diff","M_miss_square_frag_highstat_diff",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   h_M_miss_square_frag_highstat_with_accp_diff->SetLineColor(4);
   h_M_miss_square_frag_highstat_with_accp_diff->Draw();
   c_M_miss_square_frag_highstat_diff->SaveAs(Form("./reco_plot_%3.1f_%2.0f/39_M_miss_square_frag_highstat_diff.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_P_miss_frag_highstat_diff = new TCanvas("c_P_miss_frag_highstat_diff","P_miss_frag_highstat_diff",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   h_P_miss_frag_highstat_with_accp_diff->SetLineColor(4);
   h_P_miss_frag_highstat_with_accp_diff->Draw();
   c_P_miss_frag_highstat_diff->SaveAs(Form("./reco_plot_%3.1f_%2.0f/39_P_miss_frag_highstat_diff.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_P_miss_x_frag_highstat_diff = new TCanvas("c_P_miss_x_frag_highstat_diff","P_miss_x_frag_highstat_diff",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   h_P_miss_x_frag_highstat_with_accp_diff->SetLineColor(4);
   h_P_miss_x_frag_highstat_with_accp_diff->Draw();
   c_P_miss_x_frag_highstat_diff->SaveAs(Form("./reco_plot_%3.1f_%2.0f/39_P_miss_x_frag_highstat_diff.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_P_miss_y_frag_highstat_diff = new TCanvas("c_P_miss_y_frag_highstat_diff","P_miss_y_frag_highstat_diff",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   h_P_miss_y_frag_highstat_with_accp_diff->SetLineColor(4);
   h_P_miss_y_frag_highstat_with_accp_diff->Draw();
   c_P_miss_y_frag_highstat_diff->SaveAs(Form("./reco_plot_%3.1f_%2.0f/39_P_miss_y_frag_highstat_diff.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_P_miss_z_frag_highstat_diff = new TCanvas("c_P_miss_z_frag_highstat_diff","P_miss_z_frag_highstat_diff",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   h_P_miss_z_frag_highstat_with_accp_diff->SetLineColor(4);
   h_P_miss_z_frag_highstat_with_accp_diff->Draw();
   c_P_miss_z_frag_highstat_diff->SaveAs(Form("./reco_plot_%3.1f_%2.0f/39_P_miss_z_frag_highstat_diff.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_M_miss_square_frag_highstat_vs_method1 = new TCanvas("c_M_miss_square_frag_highstat_vs_method1","M_miss_square_frag_highstat_vs_method1",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   h_M_miss_square_frag_highstat_with_accp_vs_method1->SetLineColor(4);
   h_M_miss_square_frag_highstat_with_accp_vs_method1->Draw("COLZ");
   c_M_miss_square_frag_highstat_vs_method1->SaveAs(Form("./reco_plot_%3.1f_%2.0f/40_M_miss_square_frag_highstat_vs_method1.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_P_miss_frag_highstat_vs_method1 = new TCanvas("c_P_miss_frag_highstat_vs_method1","P_miss_frag_highstat_vs_method1",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   h_P_miss_frag_highstat_with_accp_vs_method1->SetLineColor(4);
   h_P_miss_frag_highstat_with_accp_vs_method1->Draw("COLZ");
   c_P_miss_frag_highstat_vs_method1->SaveAs(Form("./reco_plot_%3.1f_%2.0f/40_P_miss_frag_highstat_vs_method1.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_P_miss_x_frag_highstat_vs_method1 = new TCanvas("c_P_miss_x_frag_highstat_vs_method1","P_miss_x_frag_highstat_vs_method1",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   h_P_miss_x_frag_highstat_with_accp_vs_method1->SetLineColor(4);
   h_P_miss_x_frag_highstat_with_accp_vs_method1->Draw("COLZ");
   c_P_miss_x_frag_highstat_vs_method1->SaveAs(Form("./reco_plot_%3.1f_%2.0f/40_P_miss_x_frag_highstat_vs_method1.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_P_miss_y_frag_highstat_vs_method1 = new TCanvas("c_P_miss_y_frag_highstat_vs_method1","P_miss_y_frag_highstat_vs_method1",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   h_P_miss_y_frag_highstat_with_accp_vs_method1->SetLineColor(4);
   h_P_miss_y_frag_highstat_with_accp_vs_method1->Draw("COLZ");
   c_P_miss_y_frag_highstat_vs_method1->SaveAs(Form("./reco_plot_%3.1f_%2.0f/40_P_miss_y_frag_highstat_vs_method1.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_P_miss_z_frag_highstat_vs_method1 = new TCanvas("c_P_miss_z_frag_highstat_vs_method1","P_miss_z_frag_highstat_vs_method1",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   h_P_miss_z_frag_highstat_with_accp_vs_method1->SetLineColor(4);
   h_P_miss_z_frag_highstat_with_accp_vs_method1->Draw("COLZ");
   c_P_miss_z_frag_highstat_vs_method1->SaveAs(Form("./reco_plot_%3.1f_%2.0f/40_P_miss_z_frag_highstat_vs_method1.pdf",Boost_resolution,TOF_resolution));
   ...
   // 42
   TCanvas *c_P_miss_frag_highstat_new = new TCanvas("c_P_miss_frag_highstat_new","P_miss_frag_highstat_new",1200,800);
   gStyle->SetOptFit(11);
   gStyle->SetOptStat(11);
   h_P_miss_frag_highstat_with_accp->SetLineColor(4);
   h_P_miss_frag_highstat_with_accp->Draw();
   // h_P_miss_frag_highstat_with_accp->SetNormFactor(8478);
   // h_P_miss_frag_highstat_with_accp->Fit("gaus","","SAME",0.6,0.85);
   h_P_miss_frag_highstat_with_accp->GetYaxis()->SetRangeUser(0,N_Sim/10.*0.7);
   h_P_miss->SetLineColor(1);
   h_P_miss->Draw("Same");
   h1->SetLineColor(6);
   h1->Draw("Same");
   ...
   h5->SetNormFactor(N_Sim);
	TLegend *l_P_miss_frag_highstat_new = new TLegend(0.65,0.2,0.9,0.4,Form(""));
	l_P_miss_frag_highstat_new->SetFillColor(0);
	l_P_miss_frag_highstat_new->SetBorderSize(1);
	l_P_miss_frag_highstat_new->SetTextSize(0.03);
	l_P_miss_frag_highstat_new->SetTextFont(32);
   l_P_miss_frag_highstat_new->AddEntry(h_P_miss,"Arm-Sim","l");
   l_P_miss_frag_highstat_new->AddEntry(h_P_miss_frag_highstat_with_accp,"Frag-Sim","l");
   l_P_miss_frag_highstat_new->AddEntry(h5,"Arm-Data","l");
   l_P_miss_frag_highstat_new->AddEntry(h1,"Frag-Data","l");
   l_P_miss_frag_highstat_new->Draw();
   c_P_miss_frag_highstat_new->SaveAs(Form("./reco_plot_%3.1f_%2.0f/42_P_miss_frag_highstat.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_P_miss_x_frag_highstat_new = new TCanvas("c_P_miss_x_frag_highstat_new","P_miss_x_frag_highstat_new",1200,800);
   gStyle->SetOptFit(11);
   gStyle->SetOptStat(11);
   h_P_miss_x_frag_highstat_with_accp->SetLineColor(4);
   h_P_miss_x_frag_highstat_with_accp->Draw();
   ...
   h6->SetNormFactor(N_Sim);
	TLegend *l_P_miss_x_frag_highstat_new = new TLegend(0.65,0.2,0.9,0.4,Form(""));
	l_P_miss_x_frag_highstat_new->SetFillColor(0);
	l_P_miss_x_frag_highstat_new->SetBorderSize(1);
	l_P_miss_x_frag_highstat_new->SetTextSize(0.03);
	l_P_miss_x_frag_highstat_new->SetTextFont(32);
   l_P_miss_x_frag_highstat_new->AddEntry(h_P_miss,"Arm-Sim","l");
   l_P_miss_x_frag_highstat_new->AddEntry(h_P_miss_frag_highstat_with_accp,"Frag-Sim","l");
   l_P_miss_x_frag_highstat_new->AddEntry(h6,"Arm-Data","l");
   l_P_miss_x_frag_highstat_new->AddEntry(h2,"Frag-Data","l");
   l_P_miss_x_frag_highstat_new->Draw();
   c_P_miss_x_frag_highstat_new->SaveAs(Form("./reco_plot_%3.1f_%2.0f/42_P_miss_x_frag_highstat.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_P_miss_y_frag_highstat_new = new TCanvas("c_P_miss_y_frag_highstat_new","P_miss_y_frag_highstat_new",1200,800);
   gStyle->SetOptFit(11);
   gStyle->SetOptStat(11);
   h_P_miss_y_frag_highstat_with_accp->SetLineColor(4);
   h_P_miss_y_frag_highstat_with_accp->Draw();
   ...
   h7->SetNormFactor(N_Sim);
	TLegend *l_P_miss_y_frag_highstat_new = new TLegend(0.65,0.2,0.9,0.4,Form(""));
	l_P_miss_y_frag_highstat_new->SetFillColor(0);
	l_P_miss_y_frag_highstat_new->SetBorderSize(1);
	l_P_miss_y_frag_highstat_new->SetTextSize(0.03);
	l_P_miss_y_frag_highstat_new->SetTextFont(32);
   l_P_miss_y_frag_highstat_new->AddEntry(h_P_miss,"Arm-Sim","l");
   l_P_miss_y_frag_highstat_new->AddEntry(h_P_miss_frag_highstat_with_accp,"Frag-Sim","l");
   l_P_miss_y_frag_highstat_new->AddEntry(h7,"Arm-Data","l");
   l_P_miss_y_frag_highstat_new->AddEntry(h3,"Frag-Data","l");
   l_P_miss_y_frag_highstat_new->Draw();
   c_P_miss_y_frag_highstat_new->SaveAs(Form("./reco_plot_%3.1f_%2.0f/42_P_miss_y_frag_highstat.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_P_miss_z_frag_highstat_new = new TCanvas("c_P_miss_z_frag_highstat_new","P_miss_z_frag_highstat_new",1200,800);
   gStyle->SetOptFit(11);
   gStyle->SetOptStat(11);
   h_P_miss_z_frag_highstat_with_accp->SetLineColor(4);
   h_P_miss_z_frag_highstat_with_accp->Draw();
   h_P_miss_z_frag_highstat_with_accp->GetYaxis()->SetRangeUser(0,N_Sim/10.*1.2);
   h_p1zp2z->SetLineColor(1);
   h_p1zp2z->Draw("Same");
   h4->SetLineColor(6);
   h4->Draw("Same");
   ...
   h8->SetNormFactor(N_Sim);
	TLegend *l_P_miss_z_frag_highstat_new = new TLegend(0.65,0.2,0.9,0.4,Form(""));
	l_P_miss_z_frag_highstat_new->SetFillColor(0);
	l_P_miss_z_frag_highstat_new->SetBorderSize(1);
	l_P_miss_z_frag_highstat_new->SetTextSize(0.03);
	l_P_miss_z_frag_highstat_new->SetTextFont(32);
   l_P_miss_z_frag_highstat_new->AddEntry(h_P_miss,"Arm-Sim","l");
   l_P_miss_z_frag_highstat_new->AddEntry(h_P_miss_frag_highstat_with_accp,"Frag-Sim","l");
   l_P_miss_z_frag_highstat_new->AddEntry(h8,"Arm-Data","l");
   l_P_miss_z_frag_highstat_new->AddEntry(h4,"Frag-Data","l");
   l_P_miss_z_frag_highstat_new->Draw();
   c_P_miss_z_frag_highstat_new->SaveAs(Form("./reco_plot_%3.1f_%2.0f/42_P_miss_z_frag_highstat.pdf",Boost_resolution,TOF_resolution));

   //43
   TCanvas *c_P_miss_frag_highstat_vs_method1_new = new TCanvas("c_P_miss_frag_highstat_vs_method1_new","P_miss_frag_highstat_vs_method1_new",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   h_P_miss_frag_highstat_with_accp_vs_method1->SetLineColor(4);
   h_P_miss_frag_highstat_with_accp_vs_method1->Draw("COLZ");
   c_P_miss_frag_highstat_vs_method1_new->SaveAs(Form("./reco_plot_%3.1f_%2.0f/43_P_miss_frag_highstat_vs_method1.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_P_miss_x_frag_highstat_vs_method1_new = new TCanvas("c_P_miss_x_frag_highstat_vs_method1_new","P_miss_x_frag_highstat_vs_method1_new",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   h_P_miss_x_frag_highstat_with_accp_vs_method1->SetLineColor(4);
   h_P_miss_x_frag_highstat_with_accp_vs_method1->Draw("COLZ");
   c_P_miss_x_frag_highstat_vs_method1_new->SaveAs(Form("./reco_plot_%3.1f_%2.0f/43_P_miss_x_frag_highstat_vs_method1.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_P_miss_y_frag_highstat_vs_method1_new = new TCanvas("c_P_miss_y_frag_highstat_vs_method1_new","P_miss_y_frag_highstat_vs_method1_new",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   h_P_miss_y_frag_highstat_with_accp_vs_method1->SetLineColor(4);
   h_P_miss_y_frag_highstat_with_accp_vs_method1->Draw("COLZ");
   c_P_miss_y_frag_highstat_vs_method1_new->SaveAs(Form("./reco_plot_%3.1f_%2.0f/43_P_miss_y_frag_highstat_vs_method1.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_P_miss_z_frag_highstat_vs_method1_new = new TCanvas("c_P_miss_z_frag_highstat_vs_method1_new","P_miss_z_frag_highstat_vs_method1_new",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   h_P_miss_z_frag_highstat_with_accp_vs_method1->SetLineColor(4);
   h_P_miss_z_frag_highstat_with_accp_vs_method1->Draw("COLZ");
   c_P_miss_z_frag_highstat_vs_method1_new->SaveAs(Form("./reco_plot_%3.1f_%2.0f/43_P_miss_z_frag_highstat_vs_method1.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_P_miss_frag_highstat_vs_method1_new1 = new TCanvas("c_P_miss_frag_highstat_vs_method1_new1","P_miss_frag_highstat_vs_method1_new1",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   ...
   h9->Draw("COLZ");
   c_P_miss_frag_highstat_vs_method1_new1->SaveAs(Form("./reco_plot_%3.1f_%2.0f/43_P_miss_frag_highstat_vs_method1_data.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_P_miss_x_frag_highstat_vs_method1_new1 = new TCanvas("c_P_miss_x_frag_highstat_vs_method1_new1","P_miss_x_frag_highstat_vs_method1_new1",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   ...
   h10->Draw("COLZ");
   c_P_miss_x_frag_highstat_vs_method1_new1->SaveAs(Form("./reco_plot_%3.1f_%2.0f/43_P_miss_x_frag_highstat_vs_method1_data.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_P_miss_y_frag_highstat_vs_method1_new1 = new TCanvas("c_P_miss_y_frag_highstat_vs_method1_new1","P_miss_y_frag_highstat_vs_method1_new1",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   ...
   h11->Draw("COLZ");
   c_P_miss_y_frag_highstat_vs_method1_new1->SaveAs(Form("./reco_plot_%3.1f_%2.0f/43_P_miss_y_frag_highstat_vs_method1_data.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_P_miss_z_frag_highstat_vs_method1_new1 = new TCanvas("c_P_miss_z_frag_highstat_vs_method1_new1","P_miss_z_frag_highstat_vs_method1_new1",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   ...
   h12->Draw("COLZ");
   c_P_miss_z_frag_highstat_vs_method1_new1->SaveAs(Form("./reco_plot_%3.1f_%2.0f/43_P_miss_z_frag_highstat_vs_method1_data.pdf",Boost_resolution,TOF_resolution));

   // 44
   TCanvas *c_P_miss_frag_highstat_diff_new = new TCanvas("c_P_miss_frag_highstat_diff_new","P_miss_frag_highstat_diff_new",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   h_P_miss_frag_highstat_with_accp_diff->SetLineColor(4);
   h_P_miss_frag_highstat_with_accp_diff->Draw();
   // h_P_miss_frag_highstat_with_accp_diff->GetYaxis()->SetRangeUser(0,N_Sim/10.*1.2);
   h13->SetLineColor(6);
   h13->Draw("Same");
   h13->SetNormFactor(N_Sim);
	TLegend *l_P_miss_frag_highstat_with_accp_diff_new = new TLegend(0.65,0.2,0.9,0.4,Form(""));
	l_P_miss_frag_highstat_with_accp_diff_new->SetFillColor(0);
	l_P_miss_frag_highstat_with_accp_diff_new->SetBorderSize(1);
	l_P_miss_frag_highstat_with_accp_diff_new->SetTextSize(0.03);
	l_P_miss_frag_highstat_with_accp_diff_new->SetTextFont(32);
   l_P_miss_frag_highstat_with_accp_diff_new->AddEntry(h_P_miss_frag_highstat_with_accp_diff,"Frag-Arm(Sim)","l");
   l_P_miss_frag_highstat_with_accp_diff_new->AddEntry(h13,"Frag-Arm(Data)","l");
   l_P_miss_frag_highstat_with_accp_diff_new->Draw();
   c_P_miss_frag_highstat_diff_new->SaveAs(Form("./reco_plot_%3.1f_%2.0f/44_P_miss_frag_highstat_diff.pdf",Boost_resolution,TOF_resolution));


   TCanvas *c_P_miss_x_frag_highstat_diff_new = new TCanvas("c_P_miss_x_frag_highstat_diff_new","P_miss_x_frag_highstat_diff_new",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   h_P_miss_x_frag_highstat_with_accp_diff->SetLineColor(4);
   h_P_miss_x_frag_highstat_with_accp_diff->Draw();
   // h_P_miss_x_frag_highstat_with_accp_diff->GetYaxis()->SetRangeUser(0,N_Sim/10.*1.2);
   h14->SetLineColor(6);
   h14->Draw("Same");
   h14->SetNormFactor(N_Sim);
	TLegend *l_P_miss_x_frag_highstat_with_accp_diff_new = new TLegend(0.65,0.2,0.9,0.4,Form(""));
	l_P_miss_x_frag_highstat_with_accp_diff_new->SetFillColor(0);
	l_P_miss_x_frag_highstat_with_accp_diff_new->SetBorderSize(1);
	l_P_miss_x_frag_highstat_with_accp_diff_new->SetTextSize(0.03);
	l_P_miss_x_frag_highstat_with_accp_diff_new->SetTextFont(32);
   l_P_miss_x_frag_highstat_with_accp_diff_new->AddEntry(h_P_miss_x_frag_highstat_with_accp_diff,"Frag-Arm(Sim)","l");
   l_P_miss_x_frag_highstat_with_accp_diff_new->AddEntry(h14,"Frag-Arm(Data)","l");
   l_P_miss_x_frag_highstat_with_accp_diff_new->Draw();
   c_P_miss_x_frag_highstat_diff_new->SaveAs(Form("./reco_plot_%3.1f_%2.0f/44_P_miss_x_frag_highstat_diff.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_P_miss_y_frag_highstat_diff_new = new TCanvas("c_P_miss_y_frag_highstat_diff_new","P_miss_y_frag_highstat_diff_new",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   h_P_miss_y_frag_highstat_with_accp_diff->SetLineColor(4);
   h_P_miss_y_frag_highstat_with_accp_diff->Draw();
   // h_P_miss_y_frag_highstat_with_accp_diff->GetYaxis()->SetRangeUser(0,N_Sim/10.*1.2);
   h15->SetLineColor(6);
   h15->Draw("Same");
   h15->SetNormFactor(N_Sim);
	TLegend *l_P_miss_y_frag_highstat_with_accp_diff_new = new TLegend(0.65,0.2,0.9,0.4,Form(""));
	l_P_miss_y_frag_highstat_with_accp_diff_new->SetFillColor(0);
	l_P_miss_y_frag_highstat_with_accp_diff_new->SetBorderSize(1);
	l_P_miss_y_frag_highstat_with_accp_diff_new->SetTextSize(0.03);
	l_P_miss_y_frag_highstat_with_accp_diff_new->SetTextFont(32);
   l_P_miss_y_frag_highstat_with_accp_diff_new->AddEntry(h_P_miss_y_frag_highstat_with_accp_diff,"Frag-Arm(Sim)","l");
   l_P_miss_y_frag_highstat_with_accp_diff_new->AddEntry(h15,"Frag-Arm(Data)","l");
   l_P_miss_y_frag_highstat_with_accp_diff_new->Draw();
   c_P_miss_y_frag_highstat_diff_new->SaveAs(Form("./reco_plot_%3.1f_%2.0f/44_P_miss_y_frag_highstat_diff.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_P_miss_z_frag_highstat_diff_new = new TCanvas("c_P_miss_z_frag_highstat_diff_new","P_miss_z_frag_highstat_diff_new",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   h_P_miss_z_frag_highstat_with_accp_diff->SetLineColor(4);
   h_P_miss_z_frag_highstat_with_accp_diff->Draw();
   // h_P_miss_z_frag_highstat_with_accp_diff->GetYaxis()->SetRangeUser(0,N_Sim/10.*1.2);
   h16->SetLineColor(6);
   h16->Draw("Same");
   h16->SetNormFactor(N_Sim);
	TLegend *l_P_miss_z_frag_highstat_with_accp_diff_new = new TLegend(0.65,0.2,0.9,0.4,Form(""));
	l_P_miss_z_frag_highstat_with_accp_diff_new->SetFillColor(0);
	l_P_miss_z_frag_highstat_with_accp_diff_new->SetBorderSize(1);
	l_P_miss_z_frag_highstat_with_accp_diff_new->SetTextSize(0.03);
	l_P_miss_z_frag_highstat_with_accp_diff_new->SetTextFont(32);
   l_P_miss_z_frag_highstat_with_accp_diff_new->AddEntry(h_P_miss_z_frag_highstat_with_accp_diff,"Frag-Arm(Sim)","l");
   l_P_miss_z_frag_highstat_with_accp_diff_new->AddEntry(h16,"Frag-Arm(Data)","l");
   l_P_miss_z_frag_highstat_with_accp_diff_new->Draw();
   c_P_miss_z_frag_highstat_diff_new->SaveAs(Form("./reco_plot_%3.1f_%2.0f/44_P_miss_z_frag_highstat_diff.pdf",Boost_resolution,TOF_resolution));
   ...
   l_Pmiss_diff_ratio_new->Draw();
   c_Pmiss_diff_ratio->SaveAs(Form("./reco_plot_%3.1f_%2.0f/45_Pmiss_diff_ratio.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_Pmiss_diff_ratio_frag_highstat = new TCanvas("c_Pmiss_diff_ratio_frag_highstat","Pmiss_diff_ratio_frag_highstat",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   h_Pmiss_diff_ratio_frag_highstat->Draw("COLZ");
   c_Pmiss_diff_ratio_frag_highstat->SaveAs(Form("./reco_plot_%3.1f_%2.0f/45_Pmiss_diff_ratio_frag_highstat.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_Pmiss_diff_ratio_frag_highstat1 = new TCanvas("c_Pmiss_diff_ratio_frag_highstat1","Pmiss_diff_ratio_frag_highstat1",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   ...
   h18->Draw("COLZ");
   c_Pmiss_diff_ratio_frag_highstat1->SaveAs(Form("./reco_plot_%3.1f_%2.0f/45_Pmiss_diff_ratio_frag_highstat_data.pdf",Boost_resolution,TOF_resolution));
   ...
   // 46
   TCanvas *c_Pmiss_frag_highstat_theta = new TCanvas("c_Pmiss_frag_highstat_theta","c_Pmiss_frag_highstat_theta",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   h_Pmiss_frag_highstat_theta->Draw("COLZ");
   c_Pmiss_frag_highstat_theta->SaveAs(Form("./reco_plot_%3.1f_%2.0f/46_Pmiss_frag_highstat_theta.pdf",Boost_resolution,TOF_resolution));

   TCanvas *c_h_Pmiss_frag_highstat_theta1 = new TCanvas("c_Pmiss_frag_highstat_theta1","c_Pmiss_frag_highstat_theta1",1200,800);
   gStyle->SetOptFit(111);
   gStyle->SetOptStat(1111);
   h20->SetYTitle("Theta");
   ...
   h20->Draw("COLZ");
   c_h_Pmiss_frag_highstat_theta1->SaveAs(Form("./reco_plot_%3.1f_%2.0f/46_Pmiss_frag_highstat_theta_data.pdf",Boost_resolution,TOF_resolution));
   ...
   std::cout<<"non-QE events survives after the QE cuts: "<<NEvents_from_delta<<std::endl;
