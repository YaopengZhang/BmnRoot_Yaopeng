...
// found on file: srcsim_NoAbs_WF_100Mio.root
//////////////////////////////////////////////////////////

#ifndef Read_srcsim_NoAbs_WF_100Mio_h
#define Read_srcsim_NoAbs_WF_100Mio_h
...
class Read_srcsim_NoAbs_WF_100Mio {
public :
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   Read_srcsim_NoAbs_WF_100Mio(TTree *tree=0);
   virtual ~Read_srcsim_NoAbs_WF_100Mio();
...
#ifdef Read_srcsim_NoAbs_WF_100Mio_cxx
Read_srcsim_NoAbs_WF_100Mio::Read_srcsim_NoAbs_WF_100Mio(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("srcsim_NoAbs_WF_100Mio.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("srcsim_NoAbs_WF_100Mio.root");
      }
      // f->GetObject("bmndata",tree);

      TFile *f1 = (TFile*)gROOT->GetListOfFiles()->FindObject("srcsim_NoAbs_WF_100Mio_1.root");
      if (!f1 || !f1->IsOpen()) {
         f1 = new TFile("srcsim_NoAbs_WF_100Mio_1.root");
      }

      TFile *f2 = (TFile*)gROOT->GetListOfFiles()->FindObject("srcsim_NoAbs_WF_100Mio_2.root");
      if (!f2 || !f2->IsOpen()) {
         f2 = new TFile("srcsim_NoAbs_WF_100Mio_2.root");
      }

      // TFile *f3 = (TFile*)gROOT->GetListOfFiles()->FindObject("srcsim_NoAbs_WF_100Mio_3.root");
      // if (!f3 || !f3->IsOpen()) {
      //    f3 = new TFile("srcsim_NoAbs_WF_100Mio_3.root");
      // }
      ...
      chain->Add("srcsim_NoAbs_WF_100Mio.root");
      chain->Add("srcsim_NoAbs_WF_100Mio_1.root");
      chain->Add("srcsim_NoAbs_WF_100Mio_2.root");
      // chain->Add("srcsim_NoAbs_WF_100Mio_3.root");
...
Read_srcsim_NoAbs_WF_100Mio::~Read_srcsim_NoAbs_WF_100Mio()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t Read_srcsim_NoAbs_WF_100Mio::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t Read_srcsim_NoAbs_WF_100Mio::LoadTree(Long64_t entry)
{
...
void Read_srcsim_NoAbs_WF_100Mio::Init(TTree *tree)
{
...
Bool_t Read_srcsim_NoAbs_WF_100Mio::Notify()
{
...
void Read_srcsim_NoAbs_WF_100Mio::Show(Long64_t entry)
{
...
Int_t Read_srcsim_NoAbs_WF_100Mio::Cut(Long64_t entry)
{
...
#endif // #ifdef Read_srcsim_NoAbs_WF_100Mio_cxx
