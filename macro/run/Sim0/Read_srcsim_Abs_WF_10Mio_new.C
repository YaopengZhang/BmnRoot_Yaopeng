#define Read_srcsim_Abs_WF_10Mio_new_cxx
#include "Read_srcsim_Abs_WF_10Mio_new.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
...
// const double Mmiss2Mean = 8.77657e-01;
// const double Mmiss2Sigma = qe_mmiss2_sigmas * 1.95490e-01;

// const double qe_theta_sigmas = 2;
// const double qe_mmiss2_sigmas = 1.1;
// const double ThetaMean = 6.54994e+01;
// const double ThetaSigma = qe_theta_sigmas * 1.74660e+00;
// const double Mmiss2Mean = 7.06848e-01;
// const double Mmiss2Sigma = qe_mmiss2_sigmas * 1.82141e-01;

const double qe_theta_sigmas = 2;
const double qe_mmiss2_sigmas = 1.1;
const double ThetaMean = 6.54994e+01;
const double ThetaSigma = qe_theta_sigmas * 1.74660e+00;
// const double Mmiss2Mean = 7.80020e-01;
// const double Mmiss2Sigma = qe_mmiss2_sigmas * 1.95055e-01;
const double Mmiss2Mean = 8.90948e-01;
const double Mmiss2Sigma = qe_mmiss2_sigmas * 2.21168e-01;

// mMissMean = 7.80020e-01
// mMissSigma = qe_mmiss_sigmas * 1.95055e-01
// thetaMean = 6.54994e+01
// thetaSigma = qe_sigmas * 1.74660e+00


// mMissMean = 8.90948e-01
// mMissSigma = qe_mmiss_sigmas * 2.21168e-01
// thetaMean = 6.54994e+01
// thetaSigma = qe_sigmas * 1.74660e+00
...
double breakup_possibility(double z){
   ...
   // double sigma_tot = 900e-27; // 220 mb = 220e-27 1/cm^2
...
Double_t Read_srcsim_Abs_WF_10Mio_new::Loop(Double_t Boost_resolution, Double_t TOF_resolution)
{
//   In a ROOT session, you can do:
//      root> .L Read_srcsim_Abs_WF_10Mio_new.C
//      root> Read_srcsim_Abs_WF_10Mio_new t
...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   // ch_vertex.Add("vertex_Abs_WF_10Mio_thetaCut.root");

   // ch_vertex.Add("vertex_Abs_WF_10Mio_new.root");
   // ch_vertex.Add("vertex_Abs_WF_10Mio_thetaCut_new.root");

   ch_vertex.Add("vertex_Abs_WF_10Mio_latest.root");

   // ch_vertex.Add("vertex_Abs_WF_10Mio_new_1.root");
   // ch_vertex.Add("vertex_Abs_WF_10Mio_new_2.root");
   ...
   TH1F *h_beta_left_all = new TH1F("beta_left_all","#beta for left particle (before cuts)",100,0.7,1.);
   h_beta_left_all->SetXTitle("#beta");
	h_beta_left_all->GetXaxis()->CenterTitle(1);
	h_beta_left_all->GetXaxis()->SetTitleFont(32);
   h_beta_left_all->GetXaxis()->SetTitleSize(0.06);
	h_beta_left_all->GetXaxis()->SetTitleOffset(0.7);
   h_beta_left_all->SetYTitle("Counts");
	h_beta_left_all->GetYaxis()->CenterTitle(1);
	h_beta_left_all->GetYaxis()->SetTitleFont(32);
   h_beta_left_all->GetYaxis()->SetTitleSize(0.04);

   TH1F *h_beta_right_all = new TH1F("beta_right_all","#beta for right particle (before cuts)",100,0.7,1.);
   h_beta_right_all->SetXTitle("#beta");
	h_beta_right_all->GetXaxis()->CenterTitle(1);
	h_beta_right_all->GetXaxis()->SetTitleFont(32);
   h_beta_right_all->GetXaxis()->SetTitleSize(0.06);
	h_beta_right_all->GetXaxis()->SetTitleOffset(0.7);
   h_beta_right_all->SetYTitle("Counts");
	h_beta_right_all->GetYaxis()->CenterTitle(1);
	h_beta_right_all->GetYaxis()->SetTitleFont(32);
   h_beta_right_all->GetYaxis()->SetTitleSize(0.04);
   ...
   TFile *file_output_QE = new TFile("./QE_Simulation_Abs_WF_10Mio_new.root","RECREATE"); // "UPDATE"
   ...
   // for (Long64_t jentry=3175428; jentry<4233118;jentry++) {
      ...
      for(int i=0;i<kMaxbmnroot_0_TOF1_TOF400Point;i++){
         if(TOF400Point_fPz[i]>1e-5 && (TOF400Point_fTrackID[i]==0 || TOF400Point_fTrackID[i]==1)){
            if(TOF400Point_fPx[i]>0){
               ...
               // if(TOF_Shift==1) TOF_left = TOF400Point_fTime[i]+0.08;//ns
               // if(TOF_Shift==1) TOF_left = TOF400Point_fTime[i]-0.09;//ns
               // if(TOF_Shift==1) TOF_left = TOF400Point_fTime[i]-0.100;//ns
               ...
               // if(TOF_Shift==1) TOF_right = TOF400Point_fTime[i]+0.08;//ns
               // if(TOF_Shift==1) TOF_right = TOF400Point_fTime[i]-0.09;//ns
               // if(TOF_Shift==1) TOF_right = TOF400Point_fTime[i]-0.100;//ns
      ...
      if(Flag_left==1&&Flag_right==1){
         ...
         // h_beta_left_all->Fill(Beta_left,weight);
         // h_beta_right_all->Fill(Beta_right,weight);
      }
      ...
      if(x_left<90.||x_left>170.) Flag_left = 0;
      if(y_left<-60||y_left>60) Flag_left = 0;

      if(x_right<-170.||x_right>-90.) Flag_right = 0;
      if(y_right<-60||y_right>60) Flag_right = 0;

      if(x_left>135.&&x_left<175.&&y_left>10&&y_left<40) Flag_left = 0;
      if(x_right>-175.&&x_right<-145.&&y_right>-17&&y_right<12) Flag_right = 0;
      if(x_right>-125.&&x_right<-105.&&y_right>-62&&y_right<-42) Flag_right = 0;
      ...
      if(Flag_left==1&&Flag_right==1){
         ...
         // if(vertex_z<-575.6 - 15||vertex_z>-575.6 + 15) Continue_Flag=1;
         ...
         // if((vertex_x)*(vertex_x)+vertex_y*vertex_y>9) Continue_Flag=1;
   ...
   TFile *My_output = new TFile("./my_output_Abs_WF_10Mio_new.root", "RECREATE");
   h_vertex_z->Write();
   h_vertex_x->Write();
   h_vertex_y->Write();
   h_vertex_xy->Write();
   ...
   h_beta_right->Write();
   h_beta_left->Write();
   h_beta_right_all->Write();
   h_beta_left_all->Write();
   ...
   TFile *My_output_generator = new TFile("./my_output_generator_Abs_WF_10Mio_new.root", "RECREATE");
   h_vertex_z_1->Write();
   h_vertex_x_1->Write();
   h_vertex_y_1->Write();
   h_vertex_xy_1->Write();
