...
// found on file: srcsim_delta_test_frag_newXS.root
//////////////////////////////////////////////////////////

#ifndef Read_srcsim_delta_test_frag_newXS_h
#define Read_srcsim_delta_test_frag_newXS_h
...
class Read_srcsim_delta_test_frag_newXS {
public :
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   Read_srcsim_delta_test_frag_newXS(TTree *tree=0);
   virtual ~Read_srcsim_delta_test_frag_newXS();
...
#ifdef Read_srcsim_delta_test_frag_newXS_cxx
Read_srcsim_delta_test_frag_newXS::Read_srcsim_delta_test_frag_newXS(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("srcsim_delta_test_frag_newXS.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("srcsim_delta_test_frag_newXS.root");
      }
      // f->GetObject("bmndata",tree);
      // TFile *f2 = (TFile*)gROOT->GetListOfFiles()->FindObject("srcsim_delta_test_frag_newXS_1.root");
      // if (!f2 || !f2->IsOpen()) {
      //    f2 = new TFile("srcsim_delta_test_frag_newXS_1.root");
      // }
      ...
      chain->Add("srcsim_delta_test_frag_newXS.root");
      // chain->Add("srcsim_delta_test_frag_newXS_1.root");
...
Read_srcsim_delta_test_frag_newXS::~Read_srcsim_delta_test_frag_newXS()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t Read_srcsim_delta_test_frag_newXS::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t Read_srcsim_delta_test_frag_newXS::LoadTree(Long64_t entry)
{
...
void Read_srcsim_delta_test_frag_newXS::Init(TTree *tree)
{
...
Bool_t Read_srcsim_delta_test_frag_newXS::Notify()
{
...
void Read_srcsim_delta_test_frag_newXS::Show(Long64_t entry)
{
...
Int_t Read_srcsim_delta_test_frag_newXS::Cut(Long64_t entry)
{
...
#endif // #ifdef Read_srcsim_delta_test_frag_newXS_cxx
