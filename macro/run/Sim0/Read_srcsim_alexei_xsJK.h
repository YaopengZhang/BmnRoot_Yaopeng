...
// found on file: srcsim_alexei_xsJK.root
//////////////////////////////////////////////////////////

#ifndef Read_srcsim_alexei_xsJK_h
#define Read_srcsim_alexei_xsJK_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   Read_srcsim_alexei_xsJK(TTree *tree=0);
   virtual ~Read_srcsim_alexei_xsJK();
...
#ifdef Read_srcsim_alexei_xsJK_cxx
Read_srcsim_alexei_xsJK::Read_srcsim_alexei_xsJK(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("srcsim_alexei_xsJK.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("srcsim_alexei_xsJK.root");
      }
      // f->GetObject("bmndata",tree);
      // TFile *f2 = (TFile*)gROOT->GetListOfFiles()->FindObject("srcsim_alexei_xsJK_1.root");
      // if (!f2 || !f2->IsOpen()) {
      //    f2 = new TFile("srcsim_alexei_xsJK_1.root");
      // }

      // TFile *f3 = (TFile*)gROOT->GetListOfFiles()->FindObject("srcsim_alexei_xsJK_2.root");
      // if (!f3 || !f3->IsOpen()) {
      //    f3 = new TFile("srcsim_alexei_xsJK_2.root");
      // }

      // TFile *f4 = (TFile*)gROOT->GetListOfFiles()->FindObject("srcsim_alexei_xsJK_3.root");
      // if (!f4 || !f4->IsOpen()) {
      //    f4 = new TFile("srcsim_alexei_xsJK_3.root");
      // }
      ...
      chain->Add("srcsim_alexei_xsJK.root");
      // chain->Add("srcsim_alexei_xsJK_1.root");
      // chain->Add("srcsim_alexei_xsJK_2.root");
      // chain->Add("srcsim_alexei_xsJK_3.root");
...
Read_srcsim_alexei_xsJK::~Read_srcsim_alexei_xsJK()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t Read_srcsim_alexei_xsJK::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t Read_srcsim_alexei_xsJK::LoadTree(Long64_t entry)
{
...
void Read_srcsim_alexei_xsJK::Init(TTree *tree)
{
...
Bool_t Read_srcsim_alexei_xsJK::Notify()
{
...
void Read_srcsim_alexei_xsJK::Show(Long64_t entry)
{
...
Int_t Read_srcsim_alexei_xsJK::Cut(Long64_t entry)
{
...
#endif // #ifdef Read_srcsim_alexei_xsJK_cxx
