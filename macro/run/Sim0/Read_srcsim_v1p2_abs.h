...
// found on file: srcsim_v1p2_abs.root
//////////////////////////////////////////////////////////

#ifndef Read_srcsim_v1p2_abs_h
#define Read_srcsim_v1p2_abs_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   Read_srcsim_v1p2_abs(TTree *tree=0);
   virtual ~Read_srcsim_v1p2_abs();
...
#ifdef Read_srcsim_v1p2_abs_cxx
Read_srcsim_v1p2_abs::Read_srcsim_v1p2_abs(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("srcsim_v1p2_abs.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("srcsim_v1p2_abs.root");
      }
      ...
      chain->Add("srcsim_v1p2_abs.root");
      // chain->Add("srcsim_v1p2_abs_1.root");
      // chain->Add("srcsim_v1p2_abs_2.root");
      // chain->Add("srcsim_v1p2_abs_3.root");
...
Read_srcsim_v1p2_abs::~Read_srcsim_v1p2_abs()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t Read_srcsim_v1p2_abs::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t Read_srcsim_v1p2_abs::LoadTree(Long64_t entry)
{
...
void Read_srcsim_v1p2_abs::Init(TTree *tree)
{
...
Bool_t Read_srcsim_v1p2_abs::Notify()
{
...
void Read_srcsim_v1p2_abs::Show(Long64_t entry)
{
...
Int_t Read_srcsim_v1p2_abs::Cut(Long64_t entry)
{
...
#endif // #ifdef Read_srcsim_v1p2_abs_cxx