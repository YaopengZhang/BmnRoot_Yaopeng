...
// found on file: srcsim_delta_test_frag.root
//////////////////////////////////////////////////////////

#ifndef Read_srcsim_delta_test_frag_h
#define Read_srcsim_delta_test_frag_h
...
class Read_srcsim_delta_test_frag {
public :
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   Read_srcsim_delta_test_frag(TTree *tree=0);
   virtual ~Read_srcsim_delta_test_frag();
...
#ifdef Read_srcsim_delta_test_frag_cxx
Read_srcsim_delta_test_frag::Read_srcsim_delta_test_frag(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("srcsim_delta_test_frag.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("srcsim_delta_test_frag.root");
      }
      // f->GetObject("bmndata",tree);
      // TFile *f2 = (TFile*)gROOT->GetListOfFiles()->FindObject("srcsim_delta_test_frag_1.root");
      // if (!f2 || !f2->IsOpen()) {
      //    f2 = new TFile("srcsim_delta_test_frag_1.root");
      // }
      ...
      chain->Add("srcsim_delta_test_frag.root");
      // chain->Add("srcsim_delta_test_frag_1.root");
...
Read_srcsim_delta_test_frag::~Read_srcsim_delta_test_frag()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t Read_srcsim_delta_test_frag::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t Read_srcsim_delta_test_frag::LoadTree(Long64_t entry)
{
...
void Read_srcsim_delta_test_frag::Init(TTree *tree)
{
...
Bool_t Read_srcsim_delta_test_frag::Notify()
{
...
void Read_srcsim_delta_test_frag::Show(Long64_t entry)
{
...
Int_t Read_srcsim_delta_test_frag::Cut(Long64_t entry)
{
...
#endif // #ifdef Read_srcsim_delta_test_frag_cxx
