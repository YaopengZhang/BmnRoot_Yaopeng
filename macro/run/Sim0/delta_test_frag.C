...
void delta_test_frag(){
    ...
    TFile *f = new TFile("./vertex_delta_test_frag.root","RECREATE"); // "UPDATE"
    ...
    TFile *f_gen = new TFile("./gen_delta_test_frag.root","RECREATE"); // "UPDATE"
    ...
    ofstream OUT("Generator_input_delta_test_frag.txt"); // Generate the input file for FairAsciiGenerator
    ofstream OUT_delta("Delta_events_delta_test_frag.txt"); 
    ofstream OUT_delta_11B("Delta_events_delta_test_frag_11B.txt"); 
    
    int jentry = -1;
    int delta_flag;

    std::string directoryPath = "./DCMQGSM_SRC/DCMQGSM_Cp_3.7_AGeV_det-geom_1k/";
