#include "geometry_src_run8.C"

#define ION     // Choose generator: URQMD QGSM HSD BOX PART ION
#define GEANT4  // Choose: GEANT3 GEANT4

void geometry_src_run8_generate(TString inFile = "", TString outFile = "srcsim.root", Int_t nStartEvent = 0, Int_t nEvents = 1, Bool_t useRealEffects = kFALSE){
    TStopwatch timer;
    timer.Start();
    gDebug = 0;
    ...
    // FairIonGenerator* fIongen = new FairIonGenerator(6, 12, 6, 1, 0., 0., 4., 0., 0., -648.);
    ...
    fRun->CreateGeometryFile("geometry_src_run8.root");  // save the full setup geometry to the additional file
