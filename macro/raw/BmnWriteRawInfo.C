...
// Macro to extract information from obtained raw files to the Unified Condition Database
...
void BmnWriteRawInfo(TString input_file, TString output_file = "", UInt_t period = 7, WriteAction iAction = OnlyCreate, TString error_log_name = "")
{
    ...
    if (records > 0)
    {
        ...
        // update start time, end time and event count in the Unified Condition Database
