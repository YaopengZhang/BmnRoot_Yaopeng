    ...
    for (;;) {
        ...
        switch (fDat) {
            ...
            case SYNC_STAT:
                ...
                if (fread(data, kWORDSIZE, fDat, fRawFileIn) != fDat) continue;
                ProcessEvent(data, fDat);
                fRawTree->Fill(); fRawTreeSpills->Fill();
                /*if (evType == kBMNEOS) {
...
BmnStatus BmnRawDataDecoder::FillTDC72VXS(UInt_t *d, UInt_t serial, UInt_t &len) {
    ...
    MStreamTAI *ms0 = reinterpret_cast<MStreamTAI *> (d + index);
    index += sizeof (MStreamTAI) / kNBYTESINWORD;
    FillWR(serial, fEventId, ms0->TaiSec, ms0->TaiNSec); /*if (fEventId <= 10) { std::cout << fEventId << "\t" << sizeof (MStreamHeader) / kNBYTESINWORD << "\t" << sizeof (MStreamTAI) / kNBYTESINWORD << endl; }*/
...
BmnStatus BmnRawDataDecoder::InitDecoder() {
    ...
    if (fDetectorSetup.count(kTOF) > 0 && fDetectorSetup.at(kTOF) == 1) {
        ...
        // 3.7 mks tof400
        // 3.5 mks DCH
        if ((fPeriodId == 8) && (fBmnSetup == kSRCSETUP))
