...
BmnStatus BmnSlewingTOF700::SlewingTOF700Init() {
    ...
    fDigiTree = new TTree("bmndata", "bmndigit");
...
Int_t BmnSlewingTOF700::GetRunIdFromFile(TString name) {
    ...
    Int_t runId = 0, iter = 0; std::vector <int> execute_run_number;
    
    for (int i = name.Length() - 10; i >= 0; i--) { if (name[i] == 'n') { break; } execute_run_number.push_back(name[i] - '0'); }
    
    std::reverse(execute_run_number.begin(), execute_run_number.end());
 
    for (auto& it : execute_run_number) { runId += (int) ((double) it * TMath::Power((double) 10, (double) (execute_run_number.size() - 1 - iter))); iter ++; }
...
BmnStatus BmnSlewingTOF700::GetT0Info(Double_t& t0time, Double_t &t0width) {
    /*ofstream out("INFO.txt", std::ios::out | std::ios::app);*/
    vector<TClonesArray*>* trigArr = fTrigMapper->GetTrigArrays();
    vector<TClonesArray*>* timetrigArr;
    BmnTrigDigit* dig = 0; 
    bool is_in_T0 = false; 
    for (auto ar : *trigArr) {
    ...
    if (!is_in_T0) { t0time = 0; return kBMNERROR; }
    is_in_T0 = false;
    for (auto ar : *trigArr) {
        if (strcmp(ar->GetName(), "T0_2_A")) { continue; }
        is_in_T0 = false;
        for (int i = 0; i < ar->GetEntriesFast(); i++) {
        dig = (BmnTrigDigit*) ar->At(i);
        if (dig->GetMod() == 0) {
    ...
    if (!is_in_T0) { t0time = 0; return kBMNERROR; }
        ...
        /*if (fPeriodId > 6) { 
            if (strcmp(ar->GetName(), "BC2")) { out << ar->GetName() << endl; continue; }
        } else {
