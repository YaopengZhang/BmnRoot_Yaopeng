// macro for writing a link to the existed ROOT setup geometry for a selecting run period to the Unified Condition Database
...
void write_geometry_to_db(int geometry_link, int start_period, int start_run, int end_period, int end_run)
{
    ...
    if (geometry_link == -1)
    {
        UniqueRunNumber* pPreviousRunNumber = UniRun::FindPreviousRun(start_period, start_run);
        if (pPreviousRunNumber == nullptr)
            return;

        UniRun* pPreviosRun = UniRun::GetRun(pPreviousRunNumber->period_number, pPreviousRunNumber->run_number);
    ...
    int run_count = UniRun::GetRunNumbers(start_period, start_run, end_period, end_run, pUniqueRuns);
    if (run_count < 0)
    {
...
// macro for creating and writing a ROOT file with a full setup geometry to the Unified Condition Database and writing links for the given runs (from start to end run)
