...
std::string getRootPath()
{
    ...
    if (stat(rootPath.c_str(), &st) == 0)
        if ((st.st_mode & S_IFDIR) != 0){
            std::cerr<<"Error: DBL_FILE_PATH variable points to the folder. Please, set DBL_FILE_PATH to file path with local geometry database"<<std::endl;
            std::exit(-1);
        }
    int last = -1;
...
bool loadFairModule(FairRunSim* fRun, int dbModuleRev, const char* path, double psdx, double psdz, int iVerbose = 0) //KG psdx и psdz не используются?
{
    ...
    if (mr.active)
        ...
        command += ")";
    if (iVerbose > 0) std::cerr<<"command:"<<command<<std::endl;
    ...
    //obj->Dump();
    //std::cerr<<"dbModuleRev="<<dbModuleRev<<" idmr="<<mr.idmr<<" idm="<<mr.idm<<" class_name="<<typeid(f_cl).name()<<std::endl;
    //f_cl->Dump();
    return true;
}
...
int putMudulesRevision4SQLite(TSQLiteServer& db)
{
    ...
    if (resQ2 == 0){
        ...
        "Please, check connection to the geometry database and that DBL_FILE_PATH variable is correct"<<std::endl;
        return -1;
    }
...
int downloadModule(std::map<int,TString>* loadedModule, TString& rootDir, TSQLiteServer& db, int idsm, FairRunSim* fRun, std::map<int,SETUPMODULE>& moduleMove, int iVerbose = 0)
{
    int res = -1;
    ...
    if (iVerbose > 1) std::cerr<<"idP="<<idP<<std::endl;
    if (idP != -1){
        res = downloadModule(loadedModule, rootDir, db, idP, fRun, moduleMove, iVerbose);
        if (res < 0) return res;
    }
    if (iVerbose > 1) std::cerr<<row2->GetField(1)<<std::endl;
...
//	std::cerr<<"File path="<<stringStream.str().c_str()<<std::endl;
    ...
    if (mName == TString("CAVE") || hasEnding(stringStream.str(), ends))
        loadFairModule(fRun, dbModuleIndex, stringStream.str().c_str(), atof(row2->GetField(3)), atof(row2->GetField(5)));
    else {
        double mx = 0, my = 0, mz = 0;
        std::map<int,SETUPMODULE>::iterator itr = moduleMove.find(dbModuleIndex);
        if (iVerbose > 1) std::cerr<<"detector "<<dbModuleIndex<<std::endl;
        if (itr != moduleMove.end())
        {
            if (iVerbose > 1) std::cerr<<"detector was found"<<std::endl;
            if (itr->second.enable){
                mx = itr->second.xMove;
                my = itr->second.yMove;
                mz = itr->second.zMove;
                if (iVerbose > 1) std::cerr<<"Move detector "<<dbModuleIndex<<" to x:"<<mx<<" to y:"<<my<<" to z:"<<mz<<std::endl;
            }
        }
...
//		std::cerr<<"import file"<<stringStream.str().c_str()<<std::endl;
        ...
        if (iVerbose > 0) std::cerr<<"Exporting geometry to ROOT file: "<<out<<std::endl;
        delete geom;
        f->Close();
        delete f;
        //gGeoManager->Export(out.Data());
...
*/
    ...
    std::ostringstream stringStream;
    stringStream<<"sqlite://"<<getenv("DBL_FILE_PATH"); //KG если переменная пустая, то все отрабатывает, но неверно?
    TSQLiteServer db(stringStream.str().c_str());
    if (iVerbose > 0) std::cerr<<"Geometry Database Path: "<<stringStream.str().c_str()<<std::endl;
    stringStream.str("");

    if (putMudules4SQLite(db) != 0) return -1;
    if (putMudulesRevision4SQLite(db) != 0) return -1;

    stringStream<<"select idsd,status,idma,idfi from setup where stag='"<<tag<<"' order by revision desc;"; // KG если я правильно понял, то нужно брать по-умолчанию, самый новый релиз?
...
/*	if(idfi>0){
		...
		if(resQ==0 || resQ->GetRowCount()==0){
			...
			//dest<<"field="<<row->GetField(0)<<std::endl<<"Position=(0., 0.,"<<fieldZ<<")"<<std::endl<<"scale="<<fieldScale;
			//dest.close();
    ...
    if (idma > 0)
    {
        ...
        if (resQ == 0 || resQ->GetRowCount() == 0)
            ...
            if (iVerbose > 0) std::cout<<"Working geometry directory: "<<workP<<std::endl;
            std::size_t found = workP.find('/');
            if (found != std::string::npos){
                while ((found = workP.find('/', found+1)) != std::string::npos){
                    if (iVerbose > 1) std::cout<<"found:"<<found<<std::endl;
                    in += "../";
                }
            }
    ...
    db.Close();
    if (iVerbose > 0) std::cout<<"The setup geometry was successfully loaded"<<std::endl;
    return 0;
}

int loadSetup(const char* setupTag, const char* url)
{
    if (url == NULL) return loadSetup(setupTag, -1);
