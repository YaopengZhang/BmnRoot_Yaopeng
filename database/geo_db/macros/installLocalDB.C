...
int installLocalDB(const char* url = "http://bmn-geodb.jinr.ru")
{
    // get DBL_FILE_PATH variable with a local database filepath
    TString localDbPath = gSystem->ExpandPathName(getenv("DBL_FILE_PATH"));
    if (localDbPath == ""){
        cerr<<"DBL_FILE_PATH is not set. Please, set DBL_FILE_PATH as a path to the local database file (default, 'local.db')"<<endl;
        return -1;
    }
    std::ostringstream stringStream;
...
/*      std::ostringstream stringStream;
      ...
      cout<<"\033[31m"<<"The local database with path "<<localDbPath<<" already exists."<<stringStream.str().c_str()<<"\033[0m"<<endl;
...
/*      gSystem->Exec(TString::Format("wget -O server_revision.txt \"%s/get_cur_timestamp.php\" > /dev/null 2>&1", url));
	...
	cout <<"\033[32m"<<"The current local version of geometry database is last. Instalation is stopped."<<"\033[0m"<<std::endl;
        return 0;
      }
    ...
    if (R_ISDIR(file_stat.fMode)){
        cerr<<"DBL_FILE_PATH is a path to a folder. Please, set DBL_FILE_PATH as a path to the local database file (default, 'local.db')"<<endl;
        return -2;
    }
    ...
    gSystem->Exec(TString::Format("wget -O testdb.tar \"%s/download_file.php?type=all&tag=all\" > /dev/null 2>&1", url));
    if (gSystem->AccessPathName("testdb.tar", kFileExists))
    {
    ...
    if (fileName != "local.db")
    {
        gSystem->CopyFile(gSystem->ConcatFileName(dirName.Data(), "local.db"),
                          gSystem->ConcatFileName(dirName.Data(), fileName.Data()),
                          kTRUE);
